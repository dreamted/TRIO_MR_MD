...
typedef struct
{
	union {
		vu32 Data;
		struct {
			vu32 CEN		:1;		// rw, Counter Enable
			vu32 UPIS		:1;		// rw, Update disable(0: UEV enable): 
									//      - counter overflow/underflow
									//		- setting the UG bit
									//		- update generation through the slave mode controller
			vu32 URS		:1;		// rw, Update request source
									//   	- 0: Any of the following event or DMA request if enable
									//		- 1: Only Conter overflow/underflow generate an update or DMA request
			vu32 OPM		:1;		// rw, One pulse mode
									// 		- 0: counter is not stopped at update event
									//		- 1: counter stops counter at the next update event(clearing the bit CEN)
			vu32 DIR		:1;		// rw, direction
									// 		- 0: up-counter
									//		- 1: dn-counter
			vu32 CMS		:2;		// rw, center-aligned mode selection
									//		- 0: edge-aligned mode
									//		- 1: center-aligned mode 1
									//		- 2: center-aligned mode 2
									//		- 3: center-aligned mode 3
			vu32 ARPE		:1;		// rw, Auto-reload preload enable
									// 		- 0: TIM1_ARR register is not buffered
									//		- 1: TIM1_ARR register is buffered
			vu32 CKD		:2;		// rw, clock division
									//		- 0: by 1 tDTS = tCK_INT
									//		- 1: by 2 tDTS = 2*tCK_INT
									//		- 2: by 4 tDTS = 4*tCK_INT
		} Bit;
	...
	union {
		vu32 Data;
		struct {
			vu32 CCPC		:1;		// rw, Capture/Compare Preloaded control
									//		- 0: CCxE, CCxNE and OCxM bits are not preloaded
									//		- 1: CCxE, CCxNE and OCxM bits are preloaded
			vu32			:1;		// rw
			vu32 CCUS		:1;		// rw, capture/compare control update selection
			vu32 CCDS		:1;		// rw, caputre/compare DMA selection
									//		- 0: CCx DMA request sent when CCx event occurs
									//		- 1: CCx DMA request sent update event occurs
			vu32 MMS		:3;		// rw, Master Mode Selection
									// 		- 0: Reset
									//		- 1: Enable
									//		- 2: compare pulse
									//		- 3: compare - OC1REF
									...
									//		- 5: compare - OC3REF
									//		- 6: compare - OC4REF
			vu32 TI1S		:1;		// rw, TI1 selection
									//		- 0: TIM1_CH1 pin is connected to TI1 input
									//		- 1: TIM1_CH1, CH2, CH3 pins are connected to the TI1 input
			vu32 OIS1		:1;		// rw, output idle state 1(OC1 output)
									//		- 0: OC1 = 0 when MOE = 0
									//		- 1: OC1 = 1 when MOE = 0
			vu32 OIS1N		:1;		// rw, output idle state 1(OC1N output)
									//		- 0: OC1N = 0 when MOE = 0
									//		- 1: OC1N = 1 when MOE = 0
			vu32 OIS2		:1;		// rw, output idle state 2
			vu32 OIS2N		:1;		// rw, output idle state 2
			vu32 OIS3		:1;		// rw, output idle state 3
			vu32 OIS3N		:1;		// rw, output idle state 3
			vu32 OIS4		:1;		// rw, output idle state 4
		} Bit;
	...
	union {
		vu32 Data;
		struct {
			vu32 SMS		:3;		// rw, slave mode selection
			...
			vu32 			:1;		// rw,
			vu32 TS			:3;		// rw, trigger selection
									//		- 0: reserved
									//		- 1: TIM2(ITR1)
									//		- 2: TIM3(ITR2)
									//		- 3: TIM4(ITR3)
			...
			vu32 MSM		:1;		// rw, master/slave mode
									//		- 0: no action
									//		- 1: 
			vu32 ETF		:4;		// rw, external trigger filter
									//		- 0: no filter = fDTS
			...
			vu32 ETPS		:2;		// rw, external trigger prescaler
									// 		- 0: prescaler OFF
									//		- 1: ETRP freq. /2
									//		- 2: ETRP freq. /4
									//		- 3: ETRP freq. /8
			vu32 ECE		:1;		// rw, external clock enable
									//		- 0: external clock mode 2 disabled
									//		- 1: external clock mode 2 enabled
			vu32 ETP		:1;		// rw, external trigger ploarity
									//		- 0: ETR is non-inverted, active high lvl or rising edge
	...
	union {
		vu32 Data;
		struct {
			vu32 UIE		:1;		// rw, update interrupt enable
			vu32 CC1IE		:1;		// rw, capture/compare 1 interrupt enable
			vu32 CC2IE		:1;		// rw, capture/compare 2 interrupt enable
			vu32 CC3IE		:1;		// rw, capture/compare 3 interrupt enable
			vu32 CC4IE		:1;		// rw, capture/compare 4 interrupt enable
			vu32 COMIE		:1;		// rw, COM interrupt enable
			vu32 TIE		:1;		// rw, trigger interrupt enable
			vu32 BIE		:1;		// rw, break interrupt enable
			vu32 UDE		:1;		// rw, update DMA request enable
			vu32 CC1DE		:1;		// rw, capture/compare 1 DMA request enable
			vu32 CC2DE		:1;		// rw, capture/compare 2 DMA request enable
			vu32 CC3DE		:1;		// rw, capture/compare 3 DMA request enable
			vu32 CC4DE		:1;		// rw, caputre/compare 4 DMA request enable
			vu32 COMDE		:1;		// rw, COM DMA request enable
		} Bit;
	} DIER;	// DMA/interrupt enable regiater

	union {
		vu32 Data;
		struct {
			vu32 UIF		:1;		// rc, update interrupt flag
									//		- 0: no update occured
									//		- 1: update interrupt pending
			vu32 CC1IF		:1;		// rc, capture/compare 1 int. Flag
			vu32 CC2IF		:1;		// rc, capture/compare 2 int. Flag
			vu32 CC3IF		:1;		// rc, capture/compare 3 int. Flag
			vu32 CC4IF		:1;		// rc, capture/compare 4 int. Flag
			vu32 COMIF		:1;		// rc, COM intrrupt Flag
			vu32 TIF		:1;		// rc, Trigger interrupt Flag
			vu32 BIF		:1;		// rc, Break interrupt Flag
			vu32			:1;
			vu32 CC1OF		:1;		// rc, capture/compare 1 over capture flag
			vu32 CC2OF		:1;		// rc, capture/compare 2 over capture flag
			vu32 CC3OF		:1;		// rc, capture/compare 3 over capture flag
			vu32 CC4OF		:1;		// rc, capture/compare 4 over capture flag
		} Bit;
	} SR;	// Status register
	...
	union {
		vu32 Data;
		struct {
			vu32 UG			:1;		// w, update generation
			vu32 CC1G		:1;		// w, capture/compare 1 generation
			vu32 CC2G		:1;		// w, capture/compare 2 generation
			vu32 CC3G		:1;		// w, capture/compare 3 generation
			vu32 CC4G		:1;		// w, capture/compare 4 generation
			vu32 COM		:1;		// w, capture/compare control update generation
			vu32 TG			:1;		// w, Trigger generation
			vu32 BG			:1;		// w, Break Generation
		} Bit;
	} EGR;
	...
	union {
		vu32 Data[2];
		struct {
			vu32 CC1S		:2;		// rw,	capture/compare 1 selection
									//		- 0: CC1 channel: output
									//		- 1: CC1 channel: input, IC1 -> TI1
									//		- 2: CC1 channel: input, IC1 -> TI2
									//		- 3: CC1 channel: input, IC1 -> TRGI
			vu32 OC1FE		:1; 	// rw, output compare 1 fase enable
			vu32 OC1PE		:1; 	// rw, output compare 1 preloaded enable
			vu32 OC1M		:3; 	// rw, output compare 1 Mode
									//		- 0: frozen
									//		- 1: set channel 1 to active level on match
									//		- 2: set channel 1 to inactive level on match
									//		- 3: toggle
									//		- 4: force inactive level - OC1REF is low
									//		- 5: force active level - OC1REF is high
									//		- 6: PWM Mode 1
									//		- 7: PWM Mode 2
			vu32 OC1CE		:1; 	// rw, output compare 1 clear enable
									//		- 0: OC1Ref not affected by the ETRF input
									//		- 1: OC1Ref is cleared as soon as a high level is detected on ETRF input
			vu32 CC2S		:2; 	// rw, capture/compare 2 selection
			vu32 OC2FE		:1; 	// rw, output compare 2 fast enable
			vu32 OC2PE		:1; 	// rw, output compare 2 preloaded enable
			vu32 OC2M		:3; 	// rw, output compare 2 mode
			vu32 OC2CE		:1; 	// rw, output compare 2 cleare enable
			vu32			:16;
			
			vu32 CC3S		:2;		// rw, capture/compare 3 selection
			vu32 OC3FE		:1;		// rw, output compare 3 fast enable
			vu32 OC3PE		:1;		// rw, output compare 3 preload enable
			vu32 OC3M		:3;		// rw, output compare 3 Mode
			vu32 OC3CE		:1;		// rw, output compare 3 clear enable
			vu32 CC4S		:2;		// rw, capture/compare 4 selection
			vu32 OC4FE		:1;		// rw, output compare 4 fast enable
			vu32 OC4PE		:1;		// rw, output compare 4 preload enable
			vu32 OC4M		:3;		// rw, output compare 4 Mode
			vu32 OC4CE		:1;		// rw, output compare 4 clear enable
			vu32			:16;
		} Bit_OC;

		struct {
			vu32 CC1S		:2; 	// rw,
			vu32 IC1PSC		:2; 	// rw, input capture 1 prescaler
									//		- 0: no
									//		- 1: once 2 events
									//		- 2: once 4 events
									//		- 3: once 8 events
			vu32 IC1F		:4; 	// rw, input cature 1 filter
									//		- 0: no filter = fDTS
									//		- 1: fSAMPLING = fCK_INT, N = 2
									//		- 2: fSAMPLING = fCK_INT, N = 4
									//		- 3: fSAMPLING = fCK_INT, N = 8
									...
									//		-14: fSAMPLING = fDTS/32, N = 6
									//		-15: fSAMPLING = fDTS/32, N = 8
			vu32 CC2S		:2; 	// rw, caputure/compare 2 selection
			vu32 IC2PSC		:2; 	// rw, input capture 2 prescaler
			vu32 IC2F		:4; 	// rw, input capture 2 filter
			vu32			:16;
			
			vu32 CC3S		:2;
			vu32 IC3PSC		:2;		// rw, input capture 3 prescaler
			vu32 IC3F		:4;		// rw, input capture 3 filter
			vu32 CC4S		:2;
			vu32 IC4PSC		:2;		// rw, input capture 4 prescaler
			vu32 IC4F		:4;		// rw, input capture 4 filter
			vu32			:16;
		} Bit_IC;
	} CCMR;	// capture/compare mode register 1 & 2

	union {
		vu32 Data;
		struct {
			vu32 CC1E		:1;		// rw, capture/compare 1 output enable
			vu32 CC1P		:1;		// rw, capture/compare 1 output polarity
									//		- 0: OC1 active high
									//		- 1: OC1 active low
			vu32 CC1NE		:1;		// rw, caputre/compare 1 N-output enable
			vu32 CC1NP		:1;		// rw, capture/compare 1 N-output polarity
									//		- 0: OC1N active high
									//		- 1: OC1N active low
			vu32 CC2E		:1;		// rw, capture/compare 2 output enable
			vu32 CC2P		:1;		// rw, capture/compare 2 output polarity
			vu32 CC2NE		:1;		// rw, capture/compare 2 N-output enable
			vu32 CC2NP		:1;		// rw, capture/compare 2 N-output polarity
			vu32 CC3E		:1;		// rw, capture/compare 3 output enable
			vu32 CC3P		:1;		// rw, capture/compare 3 output polarity
			vu32 CC3NE		:1;		// rw, capture/compare 3 N-output enable
			vu32 CC3NP		:1;		// rw, capture/compare 3 N-output polarity
			vu32 CC4E		:1;		// rw, capture/compare 4 output enable
			vu32 CC4P		:1;		// rw, capture/compare 4 output polarity
		} Bit;
	} CCER;		// capture/compare enable register

	vu32 CNT;	// timer counter value
	vu32 PSC;	// prescaler register
	vu32 ARR;	// Auto-reload register(prescaler value)
	vu32 RCR;	// repertition counter register
	vu32 CCR1;	// capture/compare register 1
	vu32 CCR2;	// capture/compare register 2
	vu32 CCR3;	// capture/compare register 3
	vu32 CCR4;	// capture/compare register 4

	union {
		vu32 Data;
		struct {
			vu32 DT			:8;	// Dead-Time Generator set-up
			vu32 LOCK		:2;	// Lock configuration
			vu32 OSSI		:1;	// off-state selection for idle mode
			vu32 OSSR		:1;	// off-state selection for run mode
			vu32 BKE		:1;	// break enable
			vu32 BKP_		:1;	// break polarity
			vu32 AOE		:1;	// automatic output enable
			vu32 MOE		:1;	// main output enable
		} Bit;
	} BDTR;
	...
	union {
		...
		struct {
			vu32 DBA		:5;	// rw, DMA Base address
								// 	- 0: TIM1_CR1
								// 	- 1: TIM1_CR2
								// 	- 2: TIM1_SMCR ..
			vu32 			:3;
			vu32 DBL		:5;	// rw, DMA burst length
								//	- 0: 1bytes, 1~~...2 byte
		} Bit;
	} DCR;

	vu32 DMAR;	// DMA address for burst mode
...
// TIM1 Output Compare and PWM modes(CCMR1,2)
...
// OC MODE Value
#define __TIM1_Frozen						0
#define __TIM1_Active						1
#define __TIM1_Inative						2
#define __TIM1_Toggle						3
#define __TIM1_PWM1							6
#define __TIM1_PWM2							7
...
#define __TIM1_CKD_DIV1				0
#define __TIM1_CKD_DIV2				1
#define __TIM1_CKD_DIV4				2

// TIM1 Counter Mode (CMS + DIR)
#define __TIM1_UPCNT				0
#define __TIM1_DNDNT				1
#define __TIM1_CENTERALIGN1			2
#define __TIM1_CENTERALIGN2			4
#define __TIM1_CENTERALIGN3			6
...
void TIM1_OC1Init(int mode, int duty);
void TIM1_OC2Init(TIM1_OCInitTypeDef* TIM1_OCInitStruct);
void TIM1_OC3Init(TIM1_OCInitTypeDef* TIM1_OCInitStruct);
void TIM1_OC4Init(int mode, int duty);

void TIM1_OC1PWMOutputs(int tEn);
void TIM1_OC4PWMOutputs(int tEn);
...
void TIM1_Cmd(u32 NewState);
void TIM1_CtrlPWMOutputs(u32 Newstate);
void TIM1_ITConfig(u16 TIM1_IT, u32 NewState);
void TIM1_DMAConfig(u16 TIM1_DMABase, u16 TIM1_DMABurstLength);
void TIM1_DMACmd(u16 TIM1_DMASource, u32 Newstate);
...
void TIM1_UpdateDisableConfig(u32 Newstate);
void TIM1_UpdateRequestConfig(u8 TIM1_UpdateSource);
void TIM1_SelectHallSensor(u32 Newstate);
...
void TIM1_ARRPreloadConfig(u32 Newstate);
void TIM1_SelectCOM(u32 Newstate);
void TIM1_SelectCCDMA(u32 Newstate);
void TIM1_CCPreloadControl(u32 Newstate);
...
void TIM1_CCxCmd(u16 TIM1_Channel, u32 Newstate);
void TIM1_CCxNCmd(u16 TIM1_Channel, u32 Newstate);
