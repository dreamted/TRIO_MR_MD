...
typedef struct{
  vu32 CPUID;						// 0
  vu32 IRQControlState;				// 4
  vu32 ExceptionTableOffset;		// 8
  union {
	vu32 Data;
	struct {
  ...
  vu32 SysCtrl;						// 10
  vu32 ConfigCtrl;					// 14
  vu32 SystemPriority[3];			// 18,1C,20
  vu32 SysHandlerCtrl;				//
  vu32 ConfigFaultStatus;			// 20
...
#define SYSTEMRESET			SCB->AIRC.Data = AIRC_VECTKEY_MASK | (u32)0x04
#define CORERESET			SCB->AIRC.Data = AIRC_VECTKEY_MASK | (u32)0x01
