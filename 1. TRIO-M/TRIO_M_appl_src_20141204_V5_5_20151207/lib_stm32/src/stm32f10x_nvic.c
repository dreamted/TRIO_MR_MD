...
void NVIC_DeInit(void)
{
	...
	for(index = 0; index < 0x0B; index++) NVIC->Priority[index] = 0x00000000;
}
...
void NVIC_SCBDeInit(void)
{
    ...
    SCB->AIRC.Data = AIRC_VECTKEY_MASK;
...
void NVIC_PriorityGroupConfig(u32 NVIC_PriorityGroup)
{
  ...
  SCB->AIRC.Data = AIRC_VECTKEY_MASK | NVIC_PriorityGroup;
}
...
void NVIC_Init(u32 NVIC_IRQChannel, u32 NVIC_IRQChannelCmd)
{
	if(NVIC_IRQChannelCmd == ENABLE)
	{
		...
		NVIC->Enable[(NVIC_IRQChannel >> 0x05)] = (u32)0x01 << (NVIC_IRQChannel & (u8)0x1F);
	}
	else
	{
		// Disable the Selected IRQ Channels ---------------------------------------
		NVIC->Disable[(NVIC_IRQChannel >> 0x05)] = (u32)0x01 << (NVIC_IRQChannel & (u8)0x1F);
	}
}
...
{
	SCB->ExceptionTableOffset = Addr & 0x3FFFFF80;
}
                                      ...
                                      u8 SystemHandlerSubPriority)
{
    ...
    tmppriority = (0x700 - (SCB->AIRC.Data & (u32)0x700))>> 0x08;
