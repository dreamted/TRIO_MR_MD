...
void FLASH_HalfCycleAccessCmd(u32 FLASH_HalfCycleAccess)
{
	...
	//  FLASH->ACR.Data &= ACR_HLFCYA_Mask;
	//  FLASH->ACR.Data |= FLASH_HalfCycleAccess;
}
...
void FLASH_Unlock(void)
{
	...
	FLASH->KEYR.Data = FLASH_KEY1;
	FLASH->KEYR.Data = FLASH_KEY2;
}

INT32U FLASH_pAPBK_Check(INT32U pAddr)	// page no
{
	INT32U i = 0;
	vu32 *Addr = (vu32 *)(__pAPBKAddr + pAddr*1024);

	for(i = 0; i < (1024/4); i++)
	{
		if(Addr[i] != 0xffffffff) return TRUE;
	}
	return FALSE;
}
...
{
	INT32U i = 0;
	INT32U FLASH_Sts = FLASH_COMPLETE;


	FLASH_Unlock();
	FLASH_ClearFlag(FLASH_FLAG_BSY | FLASH_FLAG_EOP | FLASH_FLAG_PGERR | FLASH_FLAG_WRPRTERR);	

	for(i = 0; i < (APBKSIZE/1024); i++)
	{
		if(FLASH_pAPBK_Check(i))
		{
			WatchdogClear();
			FLASH_Sts = FLASH_ErasePage(__pAPBKAddr + (i << 10));
		}

		if(FLASH_Sts != FLASH_COMPLETE)
		{
			FLASH_Sts = FALSE;
			break;
		}
...
INT32U FLASH_ErasePage(u32 Page_Address)
{
	...
	{ 
		...
		FLASH->CR.Data |= CR_PER_Set;
		FLASH->AR.Data = Page_Address; 
		FLASH->CR.Data |= CR_STRT_Set;
...
INT32U FLASH_EraseOptionBytes(void)
{
  ...
  if(status == FLASH_COMPLETE)
  {
    ...
    FLASH->OPTKEYR.Data = FLASH_KEY1;
    FLASH->OPTKEYR.Data = FLASH_KEY2;
    
    /* if the previous operation is completed, proceed to erase the option bytes */
    FLASH->CR.Data |= CR_OPTER_Set;
    FLASH->CR.Data |= CR_STRT_Set;
    ...
    if(status == FLASH_COMPLETE)
    {
      ...
      FLASH->CR.Data &= CR_OPTER_Reset;
       
      /* Enable the Option Bytes Programming operation */
      FLASH->CR.Data |= CR_OPTPG_Set;
    	...
    	*(vu16*)Address = (u16)Data;
    	...
    	if(status == FLASH_COMPLETE)
	    {
			...
			*(vu16*)(Address + 2) = Data >> 16;
		    ...
		    {
				// Disable the PG Bit
				FLASH->CR.Data &= CR_PG_Reset;

				// Check Data;
				if(Data != *(vu32 *)Address) status = FLASH_ERRORWR;	// write check error
		    }
	    }
    	else
    	{
...
{
  INT32U readoutstatus = RESET;

  if ((FLASH->OBR.Data & RDPRT_Mask) != (u32)RESET)
  {
    readoutstatus = SET;
  }
  else
...
/**  
  if ((FLASH->ACR.Data & ACR_PRFTBS_Mask) != (u32)RESET)
  {
    bitstatus = SET;
  }
  else
...
void FLASH_ITConfig(u16 FLASH_IT, INT32U NewState)
{
  ...
  if(NewState != DISABLE)
  {
    ...
    FLASH->CR.Data |= FLASH_IT;
  }
  else
  {
    /* Disable the interrupt sources */
    FLASH->CR.Data &= ~(u32)FLASH_IT;
  }
}
...
INT32U FLASH_GetFlagStatus(u16 FLASH_FLAG)
{
	...
	{
		if((FLASH->OBR.Data & FLASH_FLAG_OPTERR) != (u32)RESET)
		{
		  bitstatus = SET;
		}
		else
	...
	{
		if((FLASH->SR.Data & FLASH_FLAG) != (u32)RESET)
		{
		  	bitstatus = SET;
		}
		else
...
{
  INT32U flashstatus = FLASH_COMPLETE;
  
	if((FLASH->SR.Data & FLASH_FLAG_BSY) == FLASH_FLAG_BSY) 
	{
		flashstatus = FLASH_BUSY;
	}
	else 
	{  
		if(FLASH->SR.Data & FLASH_FLAG_PGERR)
		{ 
		  flashstatus = FLASH_ERROR_PG;
		}
		else 
		{
			if(FLASH->SR.Data & FLASH_FLAG_WRPRTERR)
			{
