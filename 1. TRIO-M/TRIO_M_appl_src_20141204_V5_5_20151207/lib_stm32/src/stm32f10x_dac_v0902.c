...
void DAC1Init(void)
{
	...
	RCC->APB1ENR.Data |= (__DACEN);
	GPIOA->CNF.B4 = __DAC1;

	// DAC channel1 Configuration
	DAC->CR.B.BOFF1 = SET;
	DAC->CR.B.TSEL1 = DAC_T_NONE;
	DAC->CR.B.WAVE1 = DAC_WAVE_N;
	DAC->CR.B.MAMP1 = DAC_LFSRUnmask_Bits8_0;
	DAC->CR.B.EN1 = ENABLE;
...
void DAC2Init(void)
{
	...
	// DAC channel1 Configuration
	DAC->CR.B.BOFF2 = SET;
	DAC->CR.B.TSEL2 = DAC_T_NONE;
	DAC->CR.B.WAVE2 = DAC_WAVE_N;
	DAC->CR.B.MAMP2 = DAC_LFSRUnmask_Bits8_0;
	DAC->CR.B.EN2 = ENABLE;
	//DAC_InitStructure.DAC_Trigger = DAC_Trigger_Software;
	//DAC_InitStructure.DAC_WaveGeneration = DAC_WaveGeneration_None;
	//DAC_InitStructure.DAC_LFSRUnmask_TriangkleAmplitude = DAC_LFSRUnmask_Bits8_0;
	//DAC_InitStructure.DAC_OutputBuffer = DAC_OutputBuffer_Enable;
...
void DAC_Cmd(u32 DAC_Channel, u32 NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        DAC->CR.Data |= CR_EN_Set << DAC_Channel;
    }
    else
    {
...
void DAC_DMACmd(u32 DAC_Channel, u32 NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        DAC->CR.Data |= CR_DMAEN_Set << DAC_Channel;
    }
    else
    {
...
void DAC_SoftwareTriggerCmd(u32 DAC_Channel, u32 NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        DAC->SWTRIGR.Data |= SWTRIGR_SWTRIG_Set << (DAC_Channel >> 4);
    }
    else
    {
...
void DAC_DualSoftwareTriggerCmd(u32 NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        DAC->SWTRIGR.Data |= DUAL_SWTRIG_Set ;
    }
    else
    {
        // Disable software trigger for both DAC channels
        DAC->SWTRIGR.Data &= DUAL_SWTRIG_Reset;
    }
}
...
void DAC_WaveGenerationCmd(u32 DAC_Channel, u32 DAC_Wave, u32 NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        DAC->CR.Data |= DAC_Wave << DAC_Channel;
    }
    else
    {
...
* Return         : None
*******************************************************************************/
//	USART1Printf("return : %d %3.2f(v)\n", DAC_SetData(DAC1, srval), tval);
//	if(!flag) DAC_SetData(DAC2, 4095);
//	else	  DAC_SetData(DAC2, 0);

u16 DAC_SetData(u32 Channel, u16 Data)
{
	INT8U cnt = 10;
	// caution: 13 bit range!!
	// Channel = (Channel >> 4)*3;
	// USART1Printf("channel : %d\n", Channel);
	if(!Channel) DAC->DHR12R1 = Data;
	else		 DAC->DHR12R2 = Data;

	//DAC->DHR12RD[Channel] = Data;
	while(cnt-- && ((DAC->DOR[(Channel >> 4)] != Data)));	
	return DAC->DOR[(Channel >> 4)];
}
...
{
    // Check the parameters
	//DAC_SetChannel1Data(DAC_Align_12b_R, 0);		// for example!!
