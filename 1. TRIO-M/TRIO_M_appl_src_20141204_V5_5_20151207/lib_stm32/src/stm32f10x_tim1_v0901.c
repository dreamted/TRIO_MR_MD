...
// (PERIPH_BASE + 0x10000) + 0x2C00 - PERIPH_BASE
/* --- TIM1 CR1 Register ---*/
...
{
	int period = (SYSCLK/freq - 1);

	if(period & 0xFFFF0000) period = 0xFFFF;
	TIM1->ARR = period;
	TIM1->PSC = 0;		// default: 0
	TIM1->RCR = 0;		// default: 0
						// default: clock divistion = 0, up counter
	// TIM1 counter enable
	// set or Reset the CEN Bit
	TIM1->CR1.Bit.CEN = ENABLE;
	TIM1->BDTR.Bit.MOE = SET;		// main output enable
...
//  TIM1_OCInitStructure.TIM1_OCNIdleState = TIM1_OCIdleState_Reset;   
	...
	//  *(vu32 *) CCER_CC1E_BB = CCER_CCE_Reset;
  	TIM1->BDTR.Bit.MOE = RESET; 	  // main output enable
...
{
	TIM1->CCER.Bit.CC1E = tEn;
}

void TIM1_OC4PWMOutputs(int tEn)
{
	TIM1->CCER.Bit.CC4E = tEn;
}
...
void TIM1_OC4Init(int mode, int duty)
{
    ...
    //TIM1->CCMR2.Data = (u16)tmpccmr;
    TIM1->CCMR.Bit_OC.OC4M = mode;
    TIM1->CCER.Bit.CC4E = ENABLE;
   ...
   // *(vu32 *) CCER_CC4P_BB = TIM1_OCInitStruct->TIM1_OCPolarity;
    ...
    /* Set the Pulse value */
    // Set the Pulse value
    if(mode == __TIM1_PWM2) TIM1->CCR4 =	((TIM1->ARR)*duty)/100;
    TIM1->BDTR.Bit.MOE = SET;		// main output enable
}
...
void TIM1_TimeBaseInit(TIM1_TimeBaseInitTypeDef* TIM1_TimeBaseInitStruct)
{
 /* Check the parameters */
  ...
  TIM1->CR1.Data &= CR1_CKD_Mask & CR1_CounterMode_Mask;
  TIM1->CR1.Data |= (u32)TIM1_TimeBaseInitStruct->TIM1_ClockDivision |
...
void TIM1_TimeBaseStructInit(TIM1_TimeBaseInitTypeDef* TIM1_TimeBaseInitStruct)
{
  ...
  TIM1_TimeBaseInitStruct->TIM1_ClockDivision = __TIM1_CKD_DIV1;
  TIM1_TimeBaseInitStruct->TIM1_CounterMode = __TIM1_UPCNT;
...
void TIM1_ITConfig(u16 TIM1_IT, u32 NewState)
{
    ...
    if (NewState == ENABLE)
    {
        ...
        TIM1->DIER.Data |= TIM1_IT;
    }
    else
    {
        /* Disable the Interrupt sources */
        TIM1->DIER.Data &= (u16)~TIM1_IT;
    }
}
                             ...
                             u16 ExtTRGFilter)
{
  ...
  TIM1->SMCR.Data &= SMCR_SMS_Mask;
  TIM1->SMCR.Data |= TIM1_SlaveMode_External1;
  
  /* Select the Trigger selection : ETRF */
  TIM1->SMCR.Data &= SMCR_TS_Mask;
  TIM1->SMCR.Data |= TIM1_TS_ETRF;
}
...
void TIM1_GenerateEvent(u16 TIM1_EventSource)
{
  ...
  TIM1->EGR.Data |= TIM1_EventSource;
}
...
void TIM1_SelectOCxM(u16 TIM1_Channel, u16 TIM1_OCMode)
{
  ...
  if(TIM1_Channel == TIM1_Channel_1)
  {
    ...
    *(vu32 *) CCER_CC1E_BB = CCER_CCE_Reset;
    ...
    /* Reset the Output Compare Bits */
    TIM1->CCMR.Data[0] &= OC13Mode_Mask;

    /* Set the Ouput Compare Mode */
    TIM1->CCMR.Data[0] |= TIM1_OCMode;
  }
  else if(TIM1_Channel == TIM1_Channel_2)
  {
    /* Disable the Channel 2: Reset the CCE Bit */
    *(vu32 *) CCER_CC2E_BB = CCER_CCE_Reset;
    ...
    /* Reset the Output Compare Bits */
    TIM1->CCMR.Data[0] &= OC24Mode_Mask;

    /* Set the Ouput Compare Mode */
    TIM1->CCMR.Data[0] |= (u16)((u16)TIM1_OCMode << 8);
  }
  else if(TIM1_Channel == TIM1_Channel_3)
  {
    /* Disable the Channel 3: Reset the CCE Bit */
    *(vu32 *) CCER_CC3E_BB = CCER_CCE_Reset;
    ...
    /* Reset the Output Compare Bits */
    TIM1->CCMR.Data[1] &= OC13Mode_Mask;

    /* Set the Ouput Compare Mode */
    TIM1->CCMR.Data[1] |= TIM1_OCMode;
  }
  else
  {
    /* Disable the Channel 4: Reset the CCE Bit */
    *(vu32 *) CCER_CC4E_BB = CCER_CCE_Reset;
    ...
    /* Reset the Output Compare Bits */
    TIM1->CCMR.Data[1] &= OC24Mode_Mask;

    /* Set the Ouput Compare Mode */
    TIM1->CCMR.Data[1] |= (u16)((u16)TIM1_OCMode << 8);
  }
}
...
FlagStatus TIM1_GetFlagStatus(u16 TIM1_FLAG)
{
  ...
  if ((TIM1->SR.Data & TIM1_FLAG) != (u16)RESET )
  {
    bitstatus = SET;
  }
  else
...
ITStatus TIM1_GetITStatus(u16 TIM1_IT)
{
  ...
  TIM1_itStatus = TIM1->SR.Data & TIM1_IT;
  
  TIM1_itEnable = TIM1->DIER.Data & TIM1_IT;
