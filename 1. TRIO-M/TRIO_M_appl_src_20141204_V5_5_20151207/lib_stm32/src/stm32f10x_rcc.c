...
#define HSEStartUp_TimeOut        0xFFFF//512
...
void RCC_DeInit(void)
{
	...
	RCC->APB2RSTR.Data = 0x00000000;

	/* Disable APB1 Peripheral Reset */
	RCC->APB1RSTR.Data = 0x00000000;

	/* FLITF and SRAM Clock ON */
	RCC->AHBENR.Data = 0x00000014;

	/* Disable APB2 Peripheral Clock */
	RCC->APB2ENR.Data = 0x00000000;

	/* Disable APB1 Peripheral Clock */
	RCC->APB1ENR.Data = 0x00000000;

	// Set HSION bit
	RCC->CR.HSION = 1;
	//RCC->CR.Data |= (u32)0x00000001;

	// Reset SW[1:0], HPRE[3:0], PPRE1[2:0], PPRE2[2:0], ADCPRE[1:0] and MCO[2:0] bits
	RCC->CFGR.Data &= 0xF8FF0000;

	// Reset HSEON, CSSON and PLLON bits
	RCC->CR.HSEON = 0;
	RCC->CR.CSSON = 0;
	RCC->CR.PLLON = 0;
	//RCC->CR.Data &= 0xFEF6FFFF;

	// Reset HSEBYP bit
	RCC->CR.HSEBYP = 0;
	//RCC->CR.Data &= 0xFFFBFFFF;

	// Reset PLLSRC, PLLXTPRE, PLLMUL[3:0] and USBPRE bits
	RCC->CFGR.Data &= 0xFF80FFFF;
...
u32 RCC_WaitForHSEStartUp(int flag)
{
	...
	RCC->CR.HSEON = SET;		// external high clock enable!!
#ifdef HSE_BYPSS
		RCC->CR.HSEBYP = SET;	// external high clock bypass enable!!
#endif

	//while(StartUpCounter--);
	// Wait till HSE is ready and if Time out is reached exit

	while( !(itempreg = RCC->CR.HSERDY) && (StartUpCounter != HSEStartUp_TimeOut))
	{
		StartUpCounter++;
	}
	return ((u32)itempreg);
...
{
  return ((u8)(RCC->CFGR.Data & CFGR_SWS_Mask));
}
...
void RCC_RTCCLKConfig(u32 RCC_RTCCLKSource)
{
  ...
  RCC->BDCR.Data |= RCC_RTCCLKSource;
}
...
void RCC_GetClocksFreq(RCC_ClocksTypeDef* RCC_Clocks)
{
  ...
  /* Get SYSCLK source -------------------------------------------------------*/
  tmp = RCC->CFGR.Data & CFGR_SWS_Mask;
  ...
  switch (tmp)
  {
    ...
    case 0x08:  /* PLL used as system clock */
      ...
      pllmull = RCC->CFGR.Data & CFGR_PLLMull_Mask;
      pllmull = ( pllmull >> 18) + 2;

      pllsource = RCC->CFGR.Data & CFGR_PLLSRC_Mask;
      ...
      {/* HSE selected as PLL clock entry */
        ...
        if ((RCC->CFGR.Data & CFGR_PLLXTPRE_Mask) != (u32)RESET)
  ...
  /* Get HCLK prescaler */
  tmp = RCC->CFGR.Data & CFGR_HPRE_Set_Mask;
  tmp = tmp >> 4;
  ...
  /* Get PCLK1 prescaler */
  tmp = RCC->CFGR.Data & CFGR_PPRE1_Set_Mask;
  tmp = tmp >> 8;
  presc = APBAHBPrescTable[tmp];
  ...
  /* Get PCLK2 prescaler */
  tmp = RCC->CFGR.Data & CFGR_PPRE2_Set_Mask;
  tmp = tmp >> 11;
  presc = APBAHBPrescTable[tmp];
  ...
  tmp = RCC->CFGR.Data & CFGR_ADCPRE_Set_Mask;
  tmp = tmp >> 14;
...
void RCC_APB2PeriphResetCmd(u32 RCC_APB2Periph, INT32U NewState)
{
  ...
  if (NewState != DISABLE)
  {
    RCC->APB2RSTR.Data |= RCC_APB2Periph;
  }
  else
  {
    RCC->APB2RSTR.Data &= ~RCC_APB2Periph;
...
void RCC_APB1PeriphResetCmd(u32 RCC_APB1Periph, INT32U NewState)
{
  ...
  if (NewState != DISABLE)
  {
    RCC->APB1RSTR.Data |= RCC_APB1Periph;
  }
  else
  {
    RCC->APB1RSTR.Data &= ~RCC_APB1Periph;
...
INT32U RCC_GetFlagStatus(u8 RCC_FLAG)
{
	...
	{
		statusreg = RCC->CR.Data;
	}
	else if (tmp == 2)          /* The flag to check is in BDCR register */
	{
		statusreg = RCC->BDCR.Data;
	}
	else                       /* The flag to check is in CSR register */
	{
		statusreg = RCC->CSR.Data;
	}
