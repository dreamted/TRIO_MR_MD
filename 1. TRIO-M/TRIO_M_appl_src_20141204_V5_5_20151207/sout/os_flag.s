...
//                    urce\os_flag.c -D _MIMO_Type -lcN                       /
...
//    List file    =  D:\Project\TRIO\3.F_W\TRIO_appl_src_20120106\sout\os_fl /
//                    ag.s                                                    /
//                                                                            /
//                                                                            /
///////////////////////////////////////////////////////////////////////////////
...
// D:\Project\TRIO\3.F_W\TRIO_appl_src_20120106\uCOS-II\Source\os_flag.c
//    1 /*
//    2 *********************************************************************************************************
//    3 *                                                uC/OS-II
...
//  223     if (OSIntNesting > 0) {                         /* See if called from ISR ...                      */
        LDR.N    R0,??DataTable7  ;; OSIntNesting
        LDRB     R0,[R0, #+0]
        CMP      R0,#+0
...
//  228     pgrp = OSFlagFreeList;                          /* Get next free event flag                        */
        LDR.W    R1,??DataTable14  ;; OSFlagFreeList
        LDR      R2,[R1, #+0]
        MOVS     R6,R2
...
//  328                  pgrp->OSFlagWaitList = (void *)OSFlagFreeList; /* Return group to free list           */
        LDR.W    R0,??DataTable14  ;; OSFlagFreeList
        LDR      R1,[R0, #+0]
        STR      R1,[R4, #+4]
...
??OSFlagPend_0:
        LDR.N    R0,??Subroutine1_0  ;; OSLockNesting
        LDRB     R0,[R0, #+0]
        CMP      R0,#+0
        BEQ.N    ??OSFlagPend_2
...
??OSFlagPost_8:
        LDR      R6,[R6, #+0]
??OSFlagPost_6:
        CBZ      R6,??OSFlagPost_10
        LDRB     R0,[R6, #+18]
        CBZ      R0,??OSFlagPost_11
        CMP      R0,#+2
        BEQ.N    ??OSFlagPost_7
        BCC.N    ??OSFlagPost_12
        CMP      R0,#+3
...
// 1055     OS_MemClr((INT8U *)&OSFlagTbl[0], sizeof(OSFlagTbl));           /* Clear the flag group table      */
        LDR.N    R4,??OS_FlagInit_0  ;; OSFlagTbl
        MOVS     R1,#+140
        MOV      R0,R4
        BL       OS_MemClr
...
// 1074     OSFlagFreeList        = &OSFlagTbl[0];
        LDR.N    R0,??DataTable14  ;; OSFlagFreeList
        STR      R4,[R0, #+0]
// 1075 #endif
// 1076 }
        POP      {R4,PC}          ;; return
...
// 1113     if (ptcb->OSTCBStat == OS_STAT_RDY) {                  /* Task now ready?                          */
        LDRB     R1,[R2, #+4]
        CBNZ     R1,??OS_FlagTaskRdy_0
// 1114         OSRdyGrp               |= ptcb->OSTCBBitY;         /* Put task into ready list                 */
        LDR.N    R1,??DataTable15  ;; OSRdyGrp
        LDRB     R3,[R1, #+0]
        LDRB     R4,[R2, #+10]
        ORRS     R3,R4,R3
        STRB     R3,[R1, #+0]
        LDRB     R1,[R2, #+8]
        LDR.N    R3,??DataTable16  ;; OSRdyTbl
        LDRB     R4,[R1, R3]
        LDRB     R2,[R2, #+9]
        ORRS     R2,R2,R4
        STRB     R2,[R1, R3]
