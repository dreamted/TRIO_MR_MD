...
//                    urce\os_task.c -D _MIMO_Type -lcN                       /
...
//    List file    =  D:\Project\TRIO\3.F_W\TRIO_appl_src_20120106\sout\os_ta /
//                    sk.s                                                    /
//                                                                            /
//                                                                            /
///////////////////////////////////////////////////////////////////////////////
...
// D:\Project\TRIO\3.F_W\TRIO_appl_src_20120106\uCOS-II\Source\os_task.c
//    1 /*
//    2 *********************************************************************************************************
//    3 *                                                uC/OS-II
...
//   85     if (OSTCBPrioTbl[newprio] != (OS_TCB *)0) {                 /* New priority must not already exist */
        MOV      R12,R9
        LDR.W    R4,??DataTable25  ;; OSTCBPrioTbl
        LDR      R1,[R4, R12, LSL #+2]
        CBZ      R1,??OSTaskChangePrio_0
//   86         OS_EXIT_CRITICAL();
...
??OSTaskChangePrio_0:
        CMP      R5,#+255
        BNE.N    ??OSTaskChangePrio_1
//   90         oldprio = OSTCBCur->OSTCBPrio;                          /* Yes, get priority                   */
        LDR.W    R1,??DataTable24  ;; OSTCBCur
        LDR      R1,[R1, #+0]
        LDRB     R5,[R1, #+30]
//   91     }
...
//  116     if ((OSRdyTbl[y_old] & ptcb->OSTCBBitX) != 0) {             /* If task is ready make it not        */
        ADD      R5,R1,#+32
        LDRB     R4,[R5, #+1]
        LDR.W    R8,??DataTable22  ;; OSRdyTbl
        LDRB     R10,[R12, R8]
        TST      R10,R4
        BEQ.N    ??OSTaskChangePrio_4
//  117         OSRdyTbl[y_old] &= ~ptcb->OSTCBBitX;
        MVN      LR,R4
        ANDS     R4,LR,R10
        STRB     R4,[R12, R8]
//  118         if (OSRdyTbl[y_old] == 0) {
        LDR.W    LR,??DataTable23  ;; OSRdyGrp
        BNE.N    ??OSTaskChangePrio_5
...
//  141     if (OSRunning == OS_TRUE) {
        LDR.N    R0,??DataTable19  ;; OSRunning
        LDRB     R0,[R0, #+0]
        CMP      R0,#+1
...
??OSTaskCreate_0:
        MOV      R6,R5
        LDR.N    R7,??DataTable25  ;; OSTCBPrioTbl
        LDR      R1,[R7, R6, LSL #+2]
        CBNZ     R1,??OSTaskCreate_1
...
//  426         prio = OSTCBCur->OSTCBPrio;                             /* Set priority to delete to current   */
        LDR.N    R0,??DataTable24  ;; OSTCBCur
        LDR      R0,[R0, #+0]
        LDRB     R4,[R0, #+30]
//  427     }
//  428     ptcb = OSTCBPrioTbl[prio];
??OSTaskDel_2:
        LDR.W    R8,??DataTable25  ;; OSTCBPrioTbl
        LDR      R7,[R8, R4, LSL #+2]
...
//  463     if (OSLockNesting < 255u) {                                 /* Make sure we don't context switch   */
        LDR.N    R6,??OSTaskDel_8  ;; OSLockNesting
        LDRB     R0,[R6, #+0]
        CMP      R0,#+255
        BEQ.N    ??OSTaskDel_9
//  464         OSLockNesting++;
...
//  473     OSTaskCtr--;                                                /* One less task being managed         */
        LDR.N    R0,??OSTaskDel_8+0x4  ;; OSTaskCtr
        LDRB     R1,[R0, #+0]
        SUBS     R1,R1,#+1
        STRB     R1,[R0, #+0]
...
??OSTaskDel_12:
        LDR.N    R0,??OSTaskDel_8+0xC  ;; OSTCBFreeList
        LDR      R1,[R0, #+0]
        STR      R1,[R7, #+4]
...
//  570     ptcb = OSTCBPrioTbl[prio];
        LDR.N    R1,??DataTable25  ;; OSTCBPrioTbl
        LDR      R1,[R1, R4, LSL #+2]
//  571     if (ptcb == (OS_TCB *)0) {                                  /* Task to delete must exist           */
        CBNZ     R1,??OSTaskDelReq_2
...
//  794         if (ptcb->OSTCBStat == OS_STAT_RDY) {                 /* See if task is now ready              */
        CBNZ     R3,??OSTaskResume_3
//  795             if (ptcb->OSTCBDly == 0) {
        LDRH     R2,[R2, #+26]
        CBNZ     R2,??OSTaskResume_3
...
??OSTaskQuery_0:
        LDR.N    R0,??DataTable25  ;; OSTCBPrioTbl
        LDR      R1,[R0, R6, LSL #+2]
// 1016     if (ptcb == (OS_TCB *)0) {                   /* Task to query must exist                           */
        CBNZ     R1,??OSTaskQuery_1
