///////////////////////////////////////////////////////////////////////////////
//                                                                            /
// IAR ANSI C/C++ Compiler V5.30.2.51295/W32 for ARM    09/Jan/2012  08:12:14 /
...
//                    urce\os_mutex.c -D _MIMO_Type -lcN                      /
...
//    List file    =  D:\Project\TRIO\3.F_W\TRIO_appl_src_20120106\sout\os_mu /
//                    tex.s                                                   /
//                                                                            /
//                                                                            /
///////////////////////////////////////////////////////////////////////////////
...
// D:\Project\TRIO\3.F_W\TRIO_appl_src_20120106\uCOS-II\Source\os_mutex.c
//    1 /*
//    2 *********************************************************************************************************
//    3 *                                                uC/OS-II
...
??OSMutexAccept_0:
        LDR.W    R0,??DataTable25  ;; OSIntNesting
        LDRB     R0,[R0, #+0]
        CMP      R0,#+0
...
//  190     if (OSTCBPrioTbl[prio] != (OS_TCB *)0) {               /* Mutex priority must not already exist    */
        MOV      R1,R5
        LDR.W    R2,??DataTable24  ;; OSTCBPrioTbl
        LDR      R3,[R2, R1, LSL #+2]
        CBZ      R3,??OSMutexCreate_2
...
//  196     pevent             = OSEventFreeList;                  /* Get next free event control block        */
        LDR.N    R3,??DataTable12  ;; OSEventFreeList
        LDR      R7,[R3, #+0]
        MOVS     R6,R7
...
//  324                      OSMutex_RdyAtPrio(ptcb, prio);        /* Yes, Restore the task's original prio    */
        LDRB     R12,[R0, #+32]
        ADD      R2,R0,#+32
        LDR.W    R3,??DataTable21  ;; OSRdyTbl
        LDRB     LR,[R12, R3]
        LDRB     R6,[R2, #+1]
        MVNS     R6,R6
        ANDS     R6,R6,LR
        STRB     R6,[R12, R3]
        LDR.W    R12,??DataTable22  ;; OSRdyGrp
        BNE.N    ??OSMutexDel_11
        LDRB     LR,[R12, #+0]
        LDRB     R6,[R2, #+2]
        MVNS     R6,R6
...
??OSMutexDel_11:
        ...
        ORR      R2,R2,LR
        STRB     R2,[R6, R3]
        LDR.N    R2,??DataTable24  ;; OSTCBPrioTbl
        STR      R0,[R2, R1, LSL #+2]
//  325                  }
//  326              }
...
??OSMutexDel_12:
        ...
        LSLS     R0,R0,#+16
        LSRS     R0,R0,#+24
        LDR.N    R1,??DataTable24  ;; OSTCBPrioTbl
        MOVS     R2,#+0
        STR      R2,[R1, R0, LSL #+2]
...
??OSMutexPend_0:
        LDR.N    R0,??DataTable25  ;; OSIntNesting
        LDRB     R0,[R0, #+0]
        CBZ      R0,??OSMutexPend_2
...
??OSMutexPend_2:
        LDR.N    R0,??OSMutexPend_3  ;; OSLockNesting
        LDRB     R0,[R0, #+0]
        CBZ      R0,??OSMutexPend_4
...
??OSMutexPend_13:
        LDR.N    R0,??DataTable24  ;; OSTCBPrioTbl
        STR      R2,[R0, R1, LSL #+2]
//  493         }
//  494     }
