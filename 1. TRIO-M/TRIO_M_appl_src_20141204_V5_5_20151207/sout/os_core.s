///////////////////////////////////////////////////////////////////////////////
//                                                                            /
// IAR ANSI C/C++ Compiler V5.30.2.51295/W32 for ARM    09/Jan/2012  08:12:16 /
...
//                    urce\os_core.c -D _MIMO_Type -lcN                       /
...
//    List file    =  D:\Project\TRIO\3.F_W\TRIO_appl_src_20120106\sout\os_co /
//                    re.s                                                    /
//                                                                            /
//                                                                            /
///////////////////////////////////////////////////////////////////////////////
...
// D:\Project\TRIO\3.F_W\TRIO_appl_src_20120106\uCOS-II\Source\os_core.c
//    1 /*
//    2 *********************************************************************************************************
//    3 *                                                uC/OS-II
...
OSTime:
        ...
        SECTION `.bss`:DATA:NOROOT(2)
// OS_EVENT __data OSEventTbl[20]
OSEventTbl:
        DS8 320
// struct os_event *__data OSEventFreeList
OSEventFreeList:
        DS8 4
// OS_STK __data OSTaskIdleStk[64]
OSTaskIdleStk:
        DS8 256
// OS_TCB __data OSTCBTbl[17U]
OSTCBTbl:
        DS8 952
...
//  251     OS_InitMisc();                                               /* Initialize miscellaneous variables       */
        LDR.W    R4,??DataTable20  ;; OSIntNesting
        MOVS     R0,#+0
        STR      R0,[R4, #+36]
        STRB     R0,[R4, #+0]
        STRB     R0,[R4, #+1]
...
//  255     OS_InitTCBList();                                            /* Initialize the free list of OS_TCBs      */
        LDR.N    R5,??OSInit_0    ;; OSEventTbl
        ADD      R6,R5,#+580
        MOV      R1,#+952
        MOV      R0,R6
        BL       OS_MemClr
        MOVS     R1,#+128
        LDR.W    R0,??DataTable21  ;; OSTCBPrioTbl
        BL       OS_MemClr
        MOV      R0,R6
        ADD      R1,R5,#+636
        MOVS     R2,#+4
...
//  271     OS_InitTaskIdle();                                           /* Create the Idle Task                     */
        ...
        ADD      R2,R5,#+576
        MOVS     R1,#+0
        LDR.N    R0,??OSInit_0+0x4  ;; OS_TaskIdle
        BL       OSTaskCreate
...
OSIntEnter:
        LDR.W    R0,??DataTable20  ;; OSIntNesting
        LDRB     R1,[R0, #+5]
        CMP      R1,#+1
        BNE.N    ??OSIntEnter_0
...
//  357                     OSTCBHighRdy  = OSTCBPrioTbl[OSPrioHighRdy];
        LDR.W    R1,??DataTable21  ;; OSTCBPrioTbl
        LDR      R0,[R1, R0, LSL #+2]
        STR      R0,[R4, #+28]
...
//  476     if (OSRunning == OS_FALSE) {
        LDR.N    R4,??DataTable20  ;; OSIntNesting
        LDRB     R0,[R4, #+5]
        CBNZ     R0,??OSStart_0
...
//  479         OSTCBHighRdy  = OSTCBPrioTbl[OSPrioHighRdy]; /* Point to highest priority task ready to run    */
        LDR.N    R1,??DataTable21  ;; OSTCBPrioTbl
        LDR      R0,[R1, R0, LSL #+2]
        STR      R0,[R4, #+28]
...
//  695     y     = OSUnMapTbl[pevent->OSEventGrp];             /* Find HPT waiting for message                */
        LDR.N    R0,??DataTable18  ;; OSUnMapTbl
        LDRB     R5,[R4, #+10]
        LDRB     R6,[R5, R0]
//  696     bity  = (INT8U)(1 << y);
        MOVS     R5,#+1
...
??OS_EventTaskRdy_0:
        MOV      R4,R0
        LDR.N    R6,??DataTable21  ;; OSTCBPrioTbl
        LDR      R4,[R6, R4, LSL #+2]
...
//  732         OSRdyGrp        |=  bity;                       /* Put task in the ready to run list           */
        LDR.N    R1,??DataTable20  ;; OSIntNesting
        LDRB     R2,[R1, #+4]
        ORRS     R2,R5,R2
        STRB     R2,[R1, #+4]
//  733         OSRdyTbl[y]     |=  bitx;
...
// 1222         if (OSLockNesting == 0) {                      /* ... scheduler is not locked                  */
        LDR.N    R5,??DataTable20  ;; OSIntNesting
        LDRB     R0,[R5, #+0]
        LDRB     R1,[R5, #+1]
        ORRS     R0,R1,R0
        BNE.N    ??OS_Sched_0
...
OS_SchedNew:
        LDR.N    R0,??DataTable20  ;; OSIntNesting
        LDR.N    R1,??DataTable18  ;; OSUnMapTbl
        LDRB     R2,[R0, #+4]
        LDRB     R2,[R2, R1]
...
// 1550     ptcb = OSTCBFreeList;                                  /* Get a free TCB from the free TCB list    */
        LDR.N    R6,??DataTable20  ;; OSIntNesting
        LDR      R7,[R6, #+24]
