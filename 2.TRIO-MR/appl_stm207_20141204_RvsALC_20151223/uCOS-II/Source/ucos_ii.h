...
OS_EXT  INT32U            OSCtxSwCtr @ "iram";      			/* Counter of number of context switches           */

#if OS_EVENT_EN && (OS_MAX_EVENTS > 0)
OS_EXT  OS_EVENT         *OSEventFreeList @ "iram";				/* Pointer to list of free EVENT control blocks    */
OS_EXT  OS_EVENT          OSEventTbl[OS_MAX_EVENTS] @ "iram";	/* Table of EVENT control blocks                   */
#endif

#if (OS_FLAG_EN > 0) && (OS_MAX_FLAGS > 0)
OS_EXT  OS_FLAG_GRP       OSFlagTbl[OS_MAX_FLAGS] @ "iram";  	/* Table containing event flag groups              */
OS_EXT  OS_FLAG_GRP      *OSFlagFreeList @ "iram";           	/* Pointer to free list of event flag groups       */
#endif

#if OS_TASK_STAT_EN > 0
OS_EXT  INT8S             OSCPUUsage @ "iram";               	/* Percentage of CPU used                          */
OS_EXT  INT32U            OSIdleCtrMax @ "iram";             	/* Max. value that idle ctr can take in 1 sec.     */
OS_EXT  INT32U            OSIdleCtrRun @ "iram";             	/* Val. reached by idle ctr at run time in 1 sec.  */
OS_EXT  BOOLEAN           OSStatRdy @ "iram";                	/* Flag indicating that the statistic task is rdy  */
OS_EXT  OS_STK            OSTaskStatStk[OS_TASK_STAT_STK_SIZE] @ "iram";      /* Statistics task stack          */
#endif

OS_EXT  INT8U             OSIntNesting @ "iram";             /* Interrupt nesting level                         */

OS_EXT  INT8U             OSLockNesting @ "iram";            /* Multitasking lock nesting level                 */

OS_EXT  INT8U             OSPrioCur @ "iram";                /* Priority of current task                        */
OS_EXT  INT8U             OSPrioHighRdy @ "iram";            /* Priority of highest priority task               */

#if OS_LOWEST_PRIO <= 63
OS_EXT  INT8U             OSRdyGrp @ "iram";                        /* Ready list group                         */
OS_EXT  INT8U             OSRdyTbl[OS_RDY_TBL_SIZE] @ "iram";       /* Table of tasks which are ready to run    */
#else
OS_EXT  INT16U            OSRdyGrp @ "iram";                        /* Ready list group                         */
OS_EXT  INT16U            OSRdyTbl[OS_RDY_TBL_SIZE] @ "iram";       /* Table of tasks which are ready to run    */
#endif

OS_EXT  BOOLEAN           OSRunning @ "iram";                       /* Flag indicating that kernel is running   */

OS_EXT  INT8U             OSTaskCtr @ "iram";                       		  /* Number of tasks created                  */

OS_EXT  volatile  INT32U  OSIdleCtr @ "iram";                                 /* Idle counter                   */

OS_EXT  OS_STK            OSTaskIdleStk[OS_TASK_IDLE_STK_SIZE] @ "iram";      /* Idle task stack                */


OS_EXT  OS_TCB           *OSTCBCur @ "iram";                        		/* Pointer to currently running TCB         */
OS_EXT  OS_TCB           *OSTCBFreeList @ "iram";                   		/* Pointer to list of free TCBs             */
OS_EXT  OS_TCB           *OSTCBHighRdy @ "iram";                   			/* Pointer to highest priority TCB R-to-R   */
OS_EXT  OS_TCB           *OSTCBList @ "iram";                       		/* Pointer to doubly linked list of TCBs    */
OS_EXT  OS_TCB           *OSTCBPrioTbl[OS_LOWEST_PRIO + 1] @ "iram";		/* Table of pointers to created TCBs        */
OS_EXT  OS_TCB            OSTCBTbl[OS_MAX_TASKS + OS_N_SYS_TASKS] @ "iram"; /* Table of TCBs                  */

#if OS_TICK_STEP_EN > 0
OS_EXT  INT8U             OSTickStepState @ "iram";         		/* Indicates the state of the tick step feature    */
#endif

#if (OS_MEM_EN > 0) && (OS_MAX_MEM_PART > 0)
OS_EXT  OS_MEM           *OSMemFreeList @ "iram";            	/* Pointer to free list of memory partitions       */
OS_EXT  OS_MEM            OSMemTbl[OS_MAX_MEM_PART] @ "iram";	/* Storage for memory partition manager            */
#endif

#if (OS_Q_EN > 0) && (OS_MAX_QS > 0)
OS_EXT  OS_Q             *OSQFreeList @ "iram";              	/* Pointer to list of free QUEUE control blocks    */
OS_EXT  OS_Q              OSQTbl[OS_MAX_QS] @ "iram";        	/* Table of QUEUE control blocks                   */
#endif

#if OS_TIME_GET_SET_EN > 0
OS_EXT  volatile  INT32U  OSTime @ "iram";                   	/* Current value of system time (in ticks)         */
#endif

#if OS_TMR_EN > 0
OS_EXT  INT16U            OSTmrFree @ "iram";                	/* Number of free entries in the timer pool        */
OS_EXT  INT16U            OSTmrUsed @ "iram";                	/* Number of timers used                           */
OS_EXT  INT32U            OSTmrTime @ "iram";                	/* Current timer time                              */

OS_EXT  OS_EVENT         *OSTmrSem @ "iram";                 	/* Sem. used to gain exclusive access to timers    */
OS_EXT  OS_EVENT         *OSTmrSemSignal @ "iram";          	/* Sem. used to signal the update of timers        */

OS_EXT  OS_TMR            OSTmrTbl[OS_TMR_CFG_MAX] @ "iram"; 	/* Table containing pool of timers                 */
OS_EXT  OS_TMR           *OSTmrFreeList @ "iram";            	/* Pointer to free list of timers                  */
OS_EXT  OS_STK            OSTmrTaskStk[OS_TASK_TMR_STK_SIZE] @ "iram";

OS_EXT  OS_TMR_WHEEL      OSTmrWheelTbl[OS_TMR_CFG_WHEEL_SIZE] @ "iram";
#endif
