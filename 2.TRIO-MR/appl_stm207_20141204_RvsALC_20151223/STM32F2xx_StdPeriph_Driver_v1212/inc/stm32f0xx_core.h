...
typedef union{
	struct {
		__IO uint32_t ISER[8];                      /*!< Offset: 0x000  Interrupt Set Enable Register           (0~239)*/
		   uint32_t RESERVED0[24];                                   
	...
	struct {
		__IO uint16_t ISER[16];                      /*!< Offset: 0x000  Interrupt Set Enable Register           (0~239)*/
		   uint32_t RESERVED0[24];                                   

		__IO uint16_t ICER[16];                     /*!< Offset: 0x080  Interrupt Clear Enable Register         */
		   uint32_t RSERVED1[24];                                    

		__IO uint32_t ISPR[8];                      /*!< Offset: 0x100  Interrupt Set Pending Register          */
		   uint32_t RESERVED2[24];                                   

		__IO uint32_t ICPR[16];                     /*!< Offset: 0x180  Interrupt Clear Pending Register        */
		   uint32_t RESERVED3[24];                                   
...
typedef union
{
	...
	struct {
		union {
			vu32 Data;
			struct {
				vu32 REVISION		:4;				/*!< Implementation defined revision number */
				vu32 PARTNO			:12;			/*!< Number of processor within family */
				
				vu32				:4;
				vu32 VARIANT		:4;				/*!< Implementation defined variant number. */
				vu32 IMPLEMENTER	:8;				/*!< Implementer code. ARM is 0x41 */
			};
		} CPUID;									/*!< Offset: 0x00  CPU ID Base Register                                  */
		// 0
		union {
			vu32 Data;
			struct {
				vu32 VECTACTIVE		:9;				/*!< Active ISR number field */
				vu32				:2;
				vu32 RETTOBASE		:1;				/*!< This bit is 1 when the set of all active exceptions minus
														IPSR_current_exception yields the empty set. */
				vu32 VECTPENDING	:10;			/*!< Pending ISR number field. */

				vu32 ISRPENDING		:1;				/*!< Interrupt pending flag. Excludes NMI and Faults */
				vu32 ISRPREEMPT		:1;				/*!< You must only use this at debug time */
				vu32				:1;
				vu32 PENDSTCLR		:1;				/*!< Clear pending SysTick bit */
				vu32 PENDSTSET		:1;				/*!< Set a pending SysTick bit */
				vu32 PENDSVCLR		:1;				/*!< Clear pending pendSV */
				vu32 PENDSVSET		:1;				/*!< Set pending pendSV bit */
				vu32				:2;
				vu32 NMIPENDSET		:1;			 /*!< Set pending NMI bit */
			};
		} ICSR;			                         /*!< Offset: 0x04  Interrupt Control State Register                      */

		union {
			vu32 Data;
			struct {
				vu32				:7;
				vu32 TBLOFF			:12;		/*! < Vector table base offset field. */
				vu32 TBLBASE		:1;			/*! < Table base is in Code (0) or RAM (1) */
			};
		} VTOR;                         		/*!< Offset: 0x08  Vector Table Offset Register                          */
		union {
			vu32 Data;
			struct {
				vu32 VECTRESET		:1; 	// System Reset Bit(with the exception of debug component)
				vu32 VECTCLRACTIVE	:1; 	// Clear active vector bit
				vu32 SYSRESETREQ	:1; 	// reset(outer)
				vu32				:4;
				vu32 PRIGROUP		:4; 	// Interrupt priority grouping field
				vu32				:4; 	//
				vu32 ENDIANESS		:1; 	// Data Endianness bit(1: big endian, 0: little endian)
				vu32 VECTKEY		:16;	// register key(SET: 0x5FA, READ: 0xFA05)
			};
		} AIRCR;								/*!< Offset: 0x0C  Application Interrupt / Reset Control Register        */

		union {
			vu32 Data;
			struct {
				vu32				:1;
				vu32 SLEEPONEXIT	:1;			/*!< Sleep on exit when returning from Handler mode to Thread mode		*/
				vu32 SLEEPDEEP		:1;			/*!< Sleep deep bit */
				vu32				:1;
				vu32 SEVONPEND		:1;			/*! <When enabled, this causes WFE to wake up when an interrupt moves from inactive to
		...
		} SCR;		                     	    /*!< Offset: 0x10  System Control Register                               */
		...
		union {
			...
			struct {
				...
				vu32 USERSETMPEND	:1;
				vu32				:1;
				vu32 UNALIGN_TRP	:1;			/*!< Trap for unaligned access */
				vu32 DIV_0_TRP		:1;			/*!< Trap on Divide by 0 */
				vu32				:3;
				vu32 BFHFNMIGN		:1;
				vu32 STKALIGN		:1;			/*!< 1 = on exception entry */
			};
		} CCR;                          		/*!< Offset: 0x14  Configuration Control Register                        */
		vu32 SHP[3];                      		/*!< Offset: 0x18  System Handlers Priority Registers (4-7, 8-11, 12-15) */
