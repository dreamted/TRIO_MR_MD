...
/* Private variables ---------------------------------------------------------*/
	...
	INT8U Rx4Buff[Rx4BuffLen];
	INT8U Tx4Buff[Tx4BuffLen];

	INT16U	Rx4RdCnt = 0, Rx4WrCnt = 0;
	INT16U	Tx4RdCnt = 0, Tx4WrCnt = 0;
#else
...
// PORT INIT
	...
	RCC->__B.APB1ENR.UART4EN = ENABLE;
...
////////////////////////////////////////////////////////////////////////////////
	...
	NVIC_Init(UART4_IRQn, ENABLE); 

	USART4Printf("\n");	
}

/*******************************************************************************
  * @brief  Interrupt handler USART4 global interrupt request
  * @param  None 
  * @retval None
  *****************************************************************************/
void USART4_IRQIntHandler(void)
{
	...
	if(tmp & ( 1 << __RXNE))
	{
		Rx4Buff[Rx4WrCnt%Rx4BuffLen] = UART4->DR;		// When the receive register is read, the RDR bit is automatically cleared.
		if(++Rx4WrCnt >= Rx4BuffLen) Rx4WrCnt = 0;
	}
	else if(tmp & ( 1 << __TXE))
	{
		if(Tx4RdCnt != Tx4WrCnt)
		{
			UART4->DR = Tx4Buff[Tx4RdCnt%Tx4BuffLen];
			if(++Tx4RdCnt >= Tx4BuffLen ) Tx4RdCnt = 0;
		}
		else UART4->__B.CR1.TXEIE = DISABLE;
	}
...
#endif /* OS_INCLUDED */
	//////////////////////////////////////////////////
	Tx4Buff[Tx4WrCnt%Tx4BuffLen] = Data;
	if(++Tx4WrCnt >= Tx4BuffLen) Tx4WrCnt = 0;
	
	UART4->__B.CR1.TXEIE = ENABLE; 		
	///////////////////////////////////////////////////
#ifdef OS_INCLUDED
...
unsigned char USART4PutChar(unsigned char Data)
{
		...
		OSSemPend(pUSART4.sem, 0, &err);
	#endif /* OS_INCLUDED */
		UART4->__B.DR.DR = Data;
		while(!UART4->__B.SR.TXE);
	#ifdef OS_INCLUDED
		OSSemPost(pUSART4.sem);
	#endif /* OS_INCLUDED */
	return Data;
...
signed short int USART4RxGetByte(void)
{
	...
	RetVal = Rx4Buff[Rx4RdCnt%Rx4BuffLen];
	if(++Rx4RdCnt >= Rx4BuffLen) Rx4RdCnt = 0;
...
{
	if(Tx4RdCnt == Tx4WrCnt) return TRUE;
	else return FALSE;
}
