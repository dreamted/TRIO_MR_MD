...
void GPIOPortInit(uint16_t Reg, uint8_t GPIO_AF, GPIO_TypeDef* GPIOx, uint16_t Pin_Src)
{
	...
	GPIOx->MODER   &= ~(0x03 << Pin_Src*2);
	GPIOx->MODER   |=  (tmp.MODER << (Pin_Src*2));

	GPIOx->OTYPER  &= ~(0x01 << Pin_Src);
	GPIOx->OTYPER  |=  (tmp.OTYPER << Pin_Src);

	GPIOx->OSPEEDR &= ~(0x03 << Pin_Src*2);
	GPIOx->OSPEEDR |=  (tmp.OSPEEDR << (Pin_Src*2));

	GPIOx->PUPDR   &= ~(0x03 << Pin_Src*2);
	GPIOx->PUPDR   |=  (tmp.PUPDR << (Pin_Src*2));

	GPIO_PinAFConfig(GPIOx, Pin_Src, GPIO_AF);
