						...
						switch(s)
						{
							...
							if( (sms_len = BcMdemRecv(sms_buff, sentTel, &sms_time)) > 0)
							{
								if(0)
								{
									...
									if(FSKDebug)USART1Printf("\n");
									if(FSKDebug)USART1Printf("SendTel: %s \n", sentTel);
									if(FSKDebug)USART1Printf("sms_len: %d \n", sms_len);
								
									if(FSKDebug)USART1Printf("Arrive Time: %s %s %s %s %s %s %s\n",
...
void SmsComCheck (__ProtocolPtr *nPtr)
{
	...
	switch(Comm)	//command 체크
	{
		...
		{
	//		INT8U RetOK;
	//		RetOK = Sms_Response_Control(SmsSer,TRUE);
            Sms_Response_Control(SmsSer,TRUE);

//			USART1Printf("RetOK[%d]>[%x][%x]:[%x][%x]\n",RetOK,tDnrSts->ICCIDChecksum[0],tDnrSts->ICCIDChecksum[1],
//			EsnChecksumH,EsnChecksumL);
...
INT16U GenSmsStsAidSts_2St(INT8U *dptr, __Dnr_Sts *Sts)
{
	...
	dptr[rlen++] = Sts->BalanceModeOnOff.Data;
...
//	if((PwrAlarm.ACFail)||(PwrAlarm.DCFail)||(SetAcRecovery))	chkAlarmMaskReport = SET;
//	else														chkAlarmMaskReport = RESET;
	...
	if( chkAlarmMaskReport == RESET)
	{
		...
		{
#if 0
				 SerPtr->printf("1: RvsOutUpperLmtFail_2G[%d][%d]: [%d][%d]>[%d]>[%d]\n",	PwrAlarm.RvsOutUpperLmtFail_2G, 
			...
			if(PwrAlarmChange.RvsOutUpperLmtFail_2G)
			{
				...
				dptr[DataCnt++] = 0x00;
				PwrAlarmChange.RvsOutUpperLmtFail_2G = 0;
			}
			...
			if(PwrAlarmChange.FwdOutUpperLmtFail_2G)
			{
				...
				PwrAlarmChange.FwdOutUpperLmtFail_2G = 0;
			}
		...
		if((PwrAlarmChange.FwdAmpOnOff_LTE))
		{
			...
			if(PwrAlarmChange.RvsOutUpperLmtFail_LTE)
			{
				...
				PwrAlarmChange.RvsOutUpperLmtFail_LTE = 0;
			}
			...
			if(PwrAlarmChange.FwdOutUpperLmtFail_LTE)
			{
				...
				PwrAlarmChange.FwdOutUpperLmtFail_LTE = 0;
			}
...
//	if( Str_IsDigit(value) ) num = value - '0';
...
///////////////////////////////////////////////////////
BOOL RecieveACK (void)
{
    INT16U  nCnt=0;
    INT16U  DataCnt=0;
    
    TimerRegist ( TimerSmsRx, Time1Sec*5  );
    
    while(1)
    {
        while(!pUSART4.CheckByte(1, &nCnt))
        {
            if (TRUE == TimeOverCheck(TimerSmsRx) )
                return(FALSE);
            OSTimeDlyHMSM(0, 0, 0, 100);
        }
        DataBuff[DataCnt] = pUSART4.RxGetByte();

        ///////////////////////////////////////
        ///회선 Check
        ///////////////////////////////////////////
        if ( (DataCnt==0)&&((DataBuff[DataCnt]==0x0D)||(DataBuff[DataCnt]==0x0A)) ) continue;

//		SerPtr->printf( "\nRx Data[%d]=%c[%x]", DataCnt, DataBuff[DataCnt],DataBuff[DataCnt]);
		
        if ( DataBuff[DataCnt++] == 0x0D )
        {
			DataBuff[DataCnt++] = 0x0A;
			RecieveSMS(DataCnt);
/****
            if(DebugNmxRx[11]==TRUE)
			{
                dPrintf(" Modem Rx=");
				i_TransmitUart(UART_ENABLE, (INT8U *)DataBuff, DataCnt );
			}
****/		
            return(TRUE);
        }

		if ( DataCnt > 400 ) return(FALSE);
    }
}
...
INT16U SmsRdDataCnt = 0;
INT8U ReadSmsData(INT16U MaxLen,INT8U *Flag)
{
	if(SmsRdDataCnt >= MaxLen)
	{
		 *Flag = 0;
		 return 0xff;
	}
	else *Flag = 1;
	return DataBuff[SmsRdDataCnt++];
}


void RecieveSMS(INT16U MaxLen)
{
	INT16U i=0;
	INT8U Data=0,CheckFlag=1;
	INT8U UpData;
	BOOL  RetVal=FALSE;

	SmsRdDataCnt = 0;

	if(DownLoadGiveupTimerSet(NULL))	return;
	for ( i=0; i<5; i++ )
	{
		TimerRegist (TimerNmsRx2, Time1Sec);
		RetVal = FALSE;
		while (RetVal == FALSE)
		{
			if(CheckFlag==0) return;
			if (TRUE == TimeOverCheck(TimerNmsRx2) )
						return;
			if ( (',' == ReadSmsData(MaxLen,&CheckFlag)) )
			{	RetVal = TRUE;				
			}
		}
	}		
	...
	while (CheckFlag)
	{
		Data =ReadSmsData(MaxLen,&CheckFlag); 
		if(Data == 0x0d) 
		{
		    Rnms->CrcOK = TRUE;
			break;
		...
		{
			*((INT8U *)Rnms+i) = Ascii2Hex( UpData, Data )&0x7f;
			Rnms->SubLength = (((INT16U)Rnms->SubLen[0]<<8)&0xff00)|Rnms->SubLen[1];

			SerPtr->printf( " %2x", *((INT8U *)Rnms+i));
			
			i++;
			RetVal = FALSE;
		}
	}
//ModemDebugDataSend (Ssio,(INT8U*) DataBuff, SmsRdDataCnt-2);
...
//=====================================================================//
INT8U ResetCount=0;
void NmsTask(void * pdata)
{
	__ProtocolPtr *nPtr = (__ProtocolPtr *)SmsSer;
	...
	TimerRegist(NmsPollTimer, Time1Sec*3);
	TimerRegist(TimerTime, Time1Sec*45 );
	TimerRegist(TimerModemTime, Time1Sec*60 + Time1Sec*30 );
	TimerRegist(ModemRstTimer, Time1Sec*60L*10L);
    TimerRegist(TimerNmsBoot, Time1Sec*30L);
    
	SMS_VCC_ONOFF = _Disable;
	OSTimeDly(1500L);
	SMS_VCC_ONOFF = _Enable;

/*
	while(!tTestFlag)
		{
			if(TimeOverCheck(TimerTime))
			{
				if(ModemInit())	SerPtr->printf( "\n[%3d]ModemInit_OK\n", i++);
				else			SerPtr->printf( "\n[%3d]Fail...", i++);

				TimerRegist(TimerTime, Time1Sec);
			}
			OSTimeDly(100L);
		}
*/					
	while(!tTestFlag)
	{
        if(TimeOverCheck(TimerNmsBoot))		Past30Sec = 1;
	
		ModemOnOff = tDnrSts->ModemOnOff;
		...
		{
		    ResetCount = 0;
    		while( (Booting == 0xff) || (Booting == 0xbb) )	// 초기화가 되지 않으면 While문 안에 머뭄.
    		{
				while(tTestFlag)
				{
					OSTimeDly(100L);
...
//				UnstableRptChk();
				if(g_ubModemPwrOff == 1)	break;

				//---------------------------------------------------------------------//
    			// Modem Reset
    			if(Booting == 0xbb || TimeOverCheck(ModemRstTimer))
    			{
    				TimerRegist(ModemRstTimer, Time1Sec*60L*10L);
    				...
    				if ( ResetCount++ < 3 )
    				{
    					...
    					RecieveACK();			// Reset Receive
    					TimerRegist( TimerTime, Time1Sec*20L );
    				}
    				else if ( ResetCount++ == 3 )
    				{
    					TimerRegist( TimerModemTime, Time1Sec*60*60*24L );
    				}
    			}
				
    			if ( TimeOverCheck(TimerModemTime) )
    			{
    				ResetCount = 0;
    			}
				...
				{
//	    			for(i=0; i<3; i++)	if(ModemInit())		{ Booting = 1; SerPtr->printf( "\nModemInit_OK\n");	break; }
					if(Booting != 1)	{Booting = 0xff;	continue;}

//					for(i=0; i<3; i++)	if(ModemSPC())		{ Booting = 2; SerPtr->printf( "ModemSPC_OK\n");	break; }
	    			if(Booting != 2)	{Booting = 0xff;	continue;}
	    			for(i=0; i<3; i++)	if(ModemHangUp())	{ Booting = 3; SerPtr->printf( "ModemHangUp_OK\n");	break; }
	    			if(Booting != 3)	{Booting = 0xff;	continue;}
//	    			for(i=0; i<5; i++)	if(ModemVersion())	{ Booting = 4; SerPtr->printf( "ModemVersion_OK\n");	break; }
	    			if(Booting != 4)	{Booting = 0xff;	continue;}
//	     			for(i=0; i<5; i++)	if(ModemEsnCheck())	{ Booting = 6; SerPtr->printf( "ModemCheck_OK\n");	break; }
	    			if(Booting != 6)	{Booting = 0xff;	continue;}
//	    			for(i=0; i<5; i++)	if(ModemMinNum())	{ Booting = 7; SerPtr->printf( "ModemMinNum_OK\n");	break; }
	    			if(Booting != 7)	{Booting = 0xff;	continue;}
//	    			for(i=0; i<3; i++)	if(WhatTime())		{ Booting = 8; SerPtr->printf( "ModemTime_OK\n");	break; }
	    			if(Booting != 8)	{Booting = 0xff;	continue;}

//	    			for(i=0; i<3; i++)	if(ModemPnModeChk()) { Booting = 9; SerPtr->printf( "ModemPnModeChk_OK\n");	break; }
    			}
				else 
				{
				...
				if(Booting != 9)	{Booting = 0xff;	continue;}
				
    			if(BootFlag != 0)
				{
					for(i=0; i<ALARM_LEN; i++)
					{
						AlarmData[i] = AlarmBuffer[i];
						AlarmFlag[i] = AlarmData[i];
					}
				}

				//---------------------------------------------------------------------//
    			// Boot Alarm
    			Booting = 1;			// 부팅시 재시도 없앤다.
    			OSTimeDly(1000L);
    		}

			//SerPtr->printf("\n Modem Init Complete... Booting[%d]\n", Booting);

			//---------------------------------------------------------------------//
    		// 플래그 체크 후 함수 호출.. 
    		if( g_ubModemPwrOff == 1 )
    		{
    			g_ubModemPwrOff = 0;
                ...
                if(Booting == 1)
                {
        			...
        			for(i=0; i<3; i++)
					{ 
//						if( ModemSwPwrOffFunc() )	i=4;
					}
				}
                
				tDnrSts->ModemOnOff = _Disable;
				Booting=0;
...
//					if( WhatTime() == FALSE )
	...
	//				else ComFailCnt = 0;
					TimerRegist( TimerTime, Time1Sec*5 );
				}
			}

    		//---------------------------------------------------------------------//
    		// Boot Alarm Check
			if( BootFlag == 0  && (Booting == 1) )
			{
//				BootAlarmRptFunc();
				BootFlag = 1;
			}
...
/*
				...
				if(1)
				{
					INT8U	mode2=0, mode3=0;
					mode2 = WavTwoAutoRptChk();
					mode3 = WavThrAutoRptChk();

					if(mode2 == TRUE && mode3 == TRUE)
					{
						SpecFreqStateRspFunc(MainSts->AR_Mode[0],1,0);
					}
					else if(mode2 == TRUE)	// 현재 전파상태 보고
					{
						SpecFreqStateRspFunc(MainSts->AR_Mode[0],1,0);
					}
					else if(mode3 == TRUE)	// idle 전파상태 보고
					{
						SpecFreqStateRspFunc(MainSts->AR_Mode[1],1,1);
					}
				}

				// GUI에서 전파 측정을 요청했을때
				if( f_UserWaveCheckData_REQ == 1 )
				{
					SpecFreqStateRspFunc(MainSts->AR_Mode[1],1,0);
					f_UserWaveCheckData_REQ =0;
				}
*/
    		...
    		//---------------------------------------------------------------------//
    		// Time Check every 20 Sec
			if(!DownLoadGiveupTimerSet(NULL))  
			{
				if(TimeOverCheck(NmsPollTimer))
				{
					Rnms->Command = NULLcomm;
//					if(ModemMsgRecieveCheck())
//					{
//						NmsComCheck();
//					}
					TimerRegist( NmsPollTimer, Time1Sec*3);
    		...
    		//---------------------------------------------------------------------//
    		// DownLoad Check
    		//if(downloadFlag == (INT32U)SmsSer)
			if(DownLoadGiveupTimerSet(NULL) == (INT32U)SmsSer)
    		{
				if(nPtr->CheckByte(1, &nCnt))
        		{
  //      			if(RecieveNMS(nPtr))	NmsComCheck();
        		}

				DownLoadGiveupTimerChk(nPtr);
        	}

    		if( NmsDownErrFlag == TRUE )
    		{
    		 	INT8U   ErrMsg =0;

  //  	 		ErrMsg = ModemTcpClose();
    	 		if(ErrMsg == 2)	{Booting = 0xbb;}
 //   	 		ModemTcpExit();
    			NmsDownErrFlag = FALSE;
    		}
        }
