...
//	Ser3Ptr->printf("ADDR = %04x, nlen = %d \n", ADDR, nlen);

	for(i = 0; i < nlen; i++)
	{
		if(!(tmpAddr%BLSIZE)){
			rval = sFLASH_4kErase(Devptr, tmpAddr);//4KB erase
//			Ser3Ptr->printf("ERASE SECTOR = %d, ADDR = %08x \n", tmpAddr/BLSIZE, tmpAddr);
		}
		tmpAddr++;
	}
...
//	Ser3Ptr->printf("2. RDSTATUS [0] = %02x, [1] = %02x len = %d\n", tmp[0], tmp[1], nlen);
	//OSTimeDly(10);
...
//	pUSART3.printf("ADDR = %x \n", ADDR);
	...
	if(NeedAddr){
		...
		{
			SetSPIxCommand(Devptr->SPIx, (ADDR >> 24) & 0xFF);
			SetSPIxCommand(Devptr->SPIx, (ADDR >> 16) & 0xFF);
			SetSPIxCommand(Devptr->SPIx, (ADDR >>  8) & 0xFF);
			SetSPIxCommand(Devptr->SPIx, (ADDR >>  0) & 0xFF);
		}
