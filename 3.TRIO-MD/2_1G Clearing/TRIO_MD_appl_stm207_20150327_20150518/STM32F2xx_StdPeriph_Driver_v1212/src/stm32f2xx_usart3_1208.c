...
/* Private variables ---------------------------------------------------------*/
	...
	INT8U Rx3Buff[Rx3BuffLen];
	INT8U Tx3Buff[Tx3BuffLen];

	INT16U	Rx3RdCnt = 0, Rx3WrCnt = 0;
	INT16U	Tx3RdCnt = 0, Tx3WrCnt = 0;
#else
...
// PORT INIT
	...
	RCC->__B.APB1ENR.USART3EN = ENABLE;
...
////////////////////////////////////////////////////////////////////////////////
	...
	NVIC_Init(USART3_IRQn, ENABLE); 

	USART3Printf("\n");	
}

/*******************************************************************************
  * @brief  Interrupt handler USART3 global interrupt request
  * @param  None 
  * @retval None
  *****************************************************************************/
void USART3_IRQIntHandler(void)
{
	...
	if(tmp & ( 1 << __RXNE))
	{
		Rx3Buff[Rx3WrCnt%Rx3BuffLen] = USART3->DR;		// When the receive register is read, the RDR bit is automatically cleared.
		if(++Rx3WrCnt >= Rx3BuffLen) Rx3WrCnt = 0;
	}
	else if(tmp & ( 1 << __TXE))
	{
		if(Tx3RdCnt != Tx3WrCnt)
		{
			USART3->DR = Tx3Buff[Tx3RdCnt%Tx3BuffLen];
			if(++Tx3RdCnt >= Tx3BuffLen ) Tx3RdCnt = 0;
		}
		else USART3->__B.CR1.TXEIE = DISABLE;
	}
...
#endif /* OS_INCLUDED */
	//////////////////////////////////////////////////
	Tx3Buff[Tx3WrCnt%Tx3BuffLen] = Data;
	if(++Tx3WrCnt >= Tx3BuffLen) Tx3WrCnt = 0;
	
	USART3->__B.CR1.TXEIE = ENABLE; 		
	///////////////////////////////////////////////////
#ifdef OS_INCLUDED
...
unsigned char USART3PutChar(unsigned char Data)
{
		...
		OSSemPend(pUSART3.sem, 0, &err);
	#endif /* OS_INCLUDED */
		USART3->__B.DR.DR = Data;
		while(!USART3->__B.SR.TXE);
	#ifdef OS_INCLUDED
		OSSemPost(pUSART3.sem);
	#endif /* OS_INCLUDED */
	return Data;
...
//	char buf[128];
	char *buf = (char *)&Tx3Buff[Tx3BuffLen-256];
...
signed short int USART3RxGetByte(void)
{
	...
	RetVal = Rx3Buff[Rx3RdCnt%Rx3BuffLen];
	if(++Rx3RdCnt >= Rx3BuffLen) Rx3RdCnt = 0;
...
{
	if(Tx3RdCnt == Tx3WrCnt) return TRUE;
	else return FALSE;
}
