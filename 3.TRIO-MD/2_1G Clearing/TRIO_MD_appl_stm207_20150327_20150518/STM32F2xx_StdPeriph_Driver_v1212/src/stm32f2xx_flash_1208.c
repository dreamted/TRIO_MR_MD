...
/**
  ...
  *         1: error occurred
  */
uint32_t FLASH_If_Erase(uint32_t Addr)
{
	/* Get the sector where start the user flash area */
    if (FLASH_EraseSector(GetSector(Addr)) != FLASH_COMPLETE)
    {
...
/**
  * @brief  This function does an erase of all user flash area including unlock
  			flash
  * @param  StartSector: start of user flash area
  * @retval 0: user flash area successfully erased
  *         1: error occurred
...
uint32_t FLASH_If_EraseFunc(uint32_t Addr)
{
	uint32_t	rVal = FLASH_COMPLETE;

	FLASH_If_Init();

	if(!FLASH_If_GetWPStatus(Addr))
	{
		//SerPtr->printf("*FLASH Write Protected !!\n");

		if(FLASH_WPDisable(Addr) != 1)
		{
			//SerPtr->printf("*Error: Flash write unprotection failed\n");
			rVal = FLASH_ERROR_WRP;
			return rVal;
		}
	}

	if(FLASH_If_Erase(Addr) == FALSE)
	{
		//SerPtr->printf("*FLASH BACKUP AREA ERROR \n");
		rVal = FLASH_ERROR_ERS;
		return rVal;
	}
...
FLASH_Status FLASH_EraseSector(uint32_t FLASH_Sector)
{
	...
	{ 
		...
		FLASH->__B.CR.SER = SET;
		FLASH->__B.CR.SNB = FLASH_Sector;
...
{ 
	...
	{ 
		if(NewState != DISABLE)
		{
			BitSet(FLASH->__B.OPTCR.nWRP, OB_WRP);
			//*(__IO uint16_t*)OPTCR_BYTE2_ADDRESS &= (~OB_WRP);
		}
		else
		{
			BitRst(FLASH->__B.OPTCR.nWRP, OB_WRP);
...
/*******************************************************************************
  ...
  *****************************************************************************/
uint16_t FLASH_OB_GetWRP(uint32_t Sector)
{
	/* Return the FLASH write protection Register value */
	return ((FLASH->__B.OPTCR.nWRP >> Sector) & 0x01);
...
/*******************************************************************************
  ...
  *****************************************************************************/
uint16_t FLASH_If_GetWPStatus(uint32_t Addr)
{
	uint32_t uSector = GetSector(Addr);

	return FLASH_OB_GetWRP(uSector);

	/* 1: No write protected sectors inside the user flash area */
	/* 0: Some sectors inside the user flash area are write protected */
}
...
/*******************************************************************************
  ...
  ******************************************************************************/
uint32_t FLASH_WPDisable(uint32_t Addr)
{
	__IO uint32_t uSector = FLASH_Sector_1;

	/* Get the sector where start the user flash area */
	uSector = GetSector(Addr);
	...
	FLASH_OB_WRPConfig(uSector, DISABLE);
