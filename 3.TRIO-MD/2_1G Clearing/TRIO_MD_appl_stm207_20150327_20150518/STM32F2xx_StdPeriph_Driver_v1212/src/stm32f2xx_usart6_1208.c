...
/* Includes ------------------------------------------------------------------*/
#include "stm32f2xx_usart6_1208.h"
...
/* Private variables ---------------------------------------------------------*/
	...
	INT8U Rx6Buff[Rx6BuffLen];
	INT8U Tx6Buff[Tx6BuffLen];

	INT16U	Rx6RdCnt = 0, Rx6WrCnt = 0;
	INT16U	Tx6RdCnt = 0, Tx6WrCnt = 0;
#else
	INT8U Rx6Buff[Rx6BuffLen] @ "iram";
	INT8U Tx6Buff[Tx6BuffLen] @ "iram";

	INT16U	Rx6RdCnt @ "iram"= 0, Rx6WrCnt @ "iram" = 0;
	INT16U	Tx6RdCnt @ "iram"= 0, Tx6WrCnt @ "iram" = 0;
#endif
...
// PORT INIT
	...
	RCC->__B.APB2ENR.USART6EN = ENABLE;
	
	__PC6_U6_TX();				// USART6_TX
	__PC7_U6_RX(); 				// USART6_RX
...
#if defined (OS_INCLUDED) 
	pUSART6.sem = OSSemCreate(1);
#endif
	/* Enable USART */
	USART_Cmd(USART6, ENABLE);

	USART_ITConfig(USART6, USART_IT_RXNE, ENABLE);
	NVIC_Init(USART6_IRQn, ENABLE); 

	USART6Printf("\n");	
}

/*******************************************************************************
  * @brief  Interrupt handler USART6 global interrupt request
  * @param  None 
  * @retval None
  *****************************************************************************/
void USART6_IRQIntHandler(void)
{
	...
	if(tmp & ( 1 << __RXNE))
	{
		Rx6Buff[Rx6WrCnt%Rx6BuffLen] = USART6->DR;		// When the receive register is read, the RDR bit is automatically cleared.
		if(++Rx6WrCnt >= Rx6BuffLen) Rx6WrCnt = 0;
	}
	else if(tmp & ( 1 << __TXE))
	{
		if(Tx6RdCnt != Tx6WrCnt)
		{
			USART6->DR = Tx6Buff[Tx6RdCnt%Tx6BuffLen];
			if(++Tx6RdCnt >= Tx6BuffLen ) Tx6RdCnt = 0;
		}
		else USART6->__B.CR1.TXEIE = DISABLE;
	}
...
unsigned char USART6PutToBuffer(unsigned char Data)
{
	...
	OSSemPend(pUSART6.sem, 0, &err);
#endif /* OS_INCLUDED */
	//////////////////////////////////////////////////
	Tx6Buff[Tx6WrCnt%Tx6BuffLen] = Data;
	if(++Tx6WrCnt >= Tx6BuffLen) Tx6WrCnt = 0;
	
	USART6->__B.CR1.TXEIE = ENABLE; 		
	///////////////////////////////////////////////////
#ifdef OS_INCLUDED
	OSSemPost(pUSART6.sem);
#endif /* OS_INCLUDED */
...
unsigned char USART6PutChar(unsigned char Data)
{
		...
		OSSemPend(pUSART6.sem, 0, &err);
	#endif /* OS_INCLUDED */
		USART6->__B.DR.DR = Data;
		while(!USART6->__B.SR.TXE);
	#ifdef OS_INCLUDED
		OSSemPost(pUSART6.sem);
	#endif /* OS_INCLUDED */
	return Data;
...
unsigned long int USART6Printf( const char *format, ...)
{
	...
	for(i = 0;;i++)
	{
		if(buf[i] == 0) break;
		if(buf[i] == '\n') USART6PutToBuffer('\r');
		USART6PutToBuffer(buf[i]);
	}
	while(!USART6TxCheckByte())
	{
#ifdef OS_INCLUDED
	OSTimeDly(1);
#endif /* OS_INCLUDED */
...
#ifndef OS_INCLUDED
	while(Rx6RdCnt == Rx6WrCnt);
#else
	if(Rx6RdCnt == Rx6WrCnt) return -1;
#endif

	RetVal = Rx6Buff[Rx6RdCnt%Rx6BuffLen];
	if(++Rx6RdCnt >= Rx6BuffLen) Rx6RdCnt = 0;
