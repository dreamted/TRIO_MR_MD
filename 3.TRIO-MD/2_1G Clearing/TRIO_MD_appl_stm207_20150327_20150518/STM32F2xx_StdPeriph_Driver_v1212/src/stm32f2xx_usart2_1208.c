...
/* Private variables ---------------------------------------------------------*/
	...
	INT8U Rx2Buff[Rx2BuffLen];
	INT8U Tx2Buff[Tx2BuffLen];

	INT16U	Rx2RdCnt = 0, Rx2WrCnt = 0;
	INT16U	Tx2RdCnt = 0, Tx2WrCnt = 0;
#else
...
/**
  ...
  * @retval None
  */
void InitUSART2(INT32U nBps, INT32U nLCR)
{
...
// PORT INIT
	...
	RCC->__B.APB1ENR.USART2EN = ENABLE;
	
//	__PA2_U2_TX();				// USART2_TX
//	__PA3_U2_RX();				// USART2_RX
...
////////////////////////////////////////////////////////////////////////////////
	...
	NVIC_Init(USART2_IRQn, ENABLE); 

	USART2Printf("\n");	
}

/*******************************************************************************
  * @brief  Interrupt handler USART2 global interrupt request
  * @param  None 
  * @retval None
  *****************************************************************************/
void USART2_IRQIntHandler(void)
{

	INT16U tmp = USART2->SR;
	...
	if(tmp & ( 1 << __RXNE))
	{
		Rx2Buff[Rx2WrCnt%Rx2BuffLen] = USART2->DR;		// When the receive register is read, the RDR bit is automatically cleared.
		if(++Rx2WrCnt >= Rx2BuffLen) Rx2WrCnt = 0;
	}
	else if(tmp & ( 1 << __TXE))
	{
		if(Tx2RdCnt != Tx2WrCnt)
		{
			USART2->DR = Tx2Buff[Tx2RdCnt%Tx2BuffLen];
			if(++Tx2RdCnt >= Tx2BuffLen ) Tx2RdCnt = 0;
		}
		else USART2->__B.CR1.TXEIE = DISABLE;
	}
...
#endif /* OS_INCLUDED */
	//////////////////////////////////////////////////
	Tx2Buff[Tx2WrCnt%Tx2BuffLen] = Data;
	if(++Tx2WrCnt >= Tx2BuffLen) Tx2WrCnt = 0;
	
	USART2->__B.CR1.TXEIE = ENABLE; 		
	///////////////////////////////////////////////////
#ifdef OS_INCLUDED
...
unsigned char USART2PutChar(unsigned char Data)
{
		...
		OSSemPend(pUSART2.sem, 0, &err);
	#endif /* OS_INCLUDED */
		USART2->__B.DR.DR = Data;
		while(!USART2->__B.SR.TXE);
	#ifdef OS_INCLUDED
		OSSemPost(pUSART2.sem);
	#endif /* OS_INCLUDED */
	return Data;
...
//	char buf[128];
	char *buf = (char *)&Tx2Buff[Tx2BuffLen-256];
...
signed short int USART2RxGetByte(void)
{
	...
	RetVal = Rx2Buff[Rx2RdCnt%Rx2BuffLen];
	if(++Rx2RdCnt >= Rx2BuffLen) Rx2RdCnt = 0;
...
{
	if(Tx2RdCnt == Tx2WrCnt) return TRUE;
	else return FALSE;
}
