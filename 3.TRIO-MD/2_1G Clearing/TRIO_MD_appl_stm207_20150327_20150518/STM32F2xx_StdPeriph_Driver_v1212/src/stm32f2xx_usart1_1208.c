...
/* Private variables ---------------------------------------------------------*/
	...
	INT8U Rx1Buff[Rx1BuffLen];
	INT8U Tx1Buff[Tx1BuffLen];

	INT16U	Rx1RdCnt = 0, Rx1WrCnt = 0;
	INT16U	Tx1RdCnt = 0, Tx1WrCnt = 0;
#else
...
// PORT INIT
	...
	RCC->__B.APB2ENR.USART1EN = ENABLE;
...
////////////////////////////////////////////////////////////////////////////////
	...
	NVIC_Init(USART1_IRQn, ENABLE); 

	USART1Printf("\n");	
}

/*******************************************************************************
  * @brief  Interrupt handler USART1 global interrupt request
  * @param  None 
  * @retval None
  *****************************************************************************/
void USART1_IRQIntHandler(void)
{
	INT16U tmp = USART1->SR;
	...
	if(tmp & ( 1 << __RXNE))
	{
		Rx1Buff[Rx1WrCnt%Rx1BuffLen] = USART1->DR;		// When the receive register is read, the RDR bit is automatically cleared.
		if(++Rx1WrCnt >= Rx1BuffLen) Rx1WrCnt = 0;
	}
	else if(tmp & ( 1 << __TXE))
	{
		if(Tx1RdCnt != Tx1WrCnt)
		{
			USART1->DR = Tx1Buff[Tx1RdCnt%Tx1BuffLen];
			if(++Tx1RdCnt >= Tx1BuffLen ) Tx1RdCnt = 0;
		}
		else USART1->__B.CR1.TXEIE = DISABLE;
	}
...
#endif /* OS_INCLUDED */
	//////////////////////////////////////////////////
	Tx1Buff[Tx1WrCnt%Tx1BuffLen] = Data;
	if(++Tx1WrCnt >= Tx1BuffLen) Tx1WrCnt = 0;
	
	USART1->__B.CR1.TXEIE = ENABLE; 		
	///////////////////////////////////////////////////
#ifdef OS_INCLUDED
...
unsigned char USART1PutChar(unsigned char Data)
{
		...
		OSSemPend(pUSART1.sem, 0, &err);
	#endif /* OS_INCLUDED */
		USART1->__B.DR.DR = Data;
		while(!USART1->__B.SR.TXE);
	#ifdef OS_INCLUDED
		OSSemPost(pUSART1.sem);
	#endif /* OS_INCLUDED */
	return Data;
...
signed short int USART1RxGetByte(void)
{
	...
	RetVal = Rx1Buff[Rx1RdCnt%Rx1BuffLen];
	if(++Rx1RdCnt >= Rx1BuffLen) Rx1RdCnt = 0;
...
{
	if(Tx1RdCnt == Tx1WrCnt) return TRUE;
	else return FALSE;
}
