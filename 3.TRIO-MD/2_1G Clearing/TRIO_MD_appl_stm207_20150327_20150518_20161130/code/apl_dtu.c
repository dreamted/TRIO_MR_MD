...
//	if(_DTU_Insert == __RESET)
	...
	if(!IsItDTUInited)		//DTU ÃÊ±âÈ­
	{
		...
		if(DTU_ReadData(CS0SEL, DTU_STATUS) != INIT_CHECK_DATA){
			...
			SerPtr->printf("FPGA Version[%x][%x][%x]>[%x]\n",tDnrSts->FPGAVer[0] ,tDnrSts->VendorID ,tDnrSts->UnitID,tDnrSts->BandFilterInfo);
		...
		//	OSTimeDly(500L*3L);
			...
			SerPtr->printf("BandFilterInfo[%x]\n",tDnrSts->FPGAVer[0] ,tDnrSts->VendorID ,tDnrSts->UnitID,tDnrSts->BandFilterInfo);
		}
	}
...
void FilterCoefficientWrite(INT8U Path)
{
	...
	OSTimeDly(500L);
#if 0
	pUSART1.printf("FilterCoefficientWrite:[%d]: ",Path);
	pUSART1.printf("0x%02x,",table[Path].Path);
	pUSART1.printf("0x%02x,",table[Path].DLFreg_Sel);
	pUSART1.printf("0x%02x,",table[Path].DLFreg_Data[0]);
	pUSART1.printf("0x%02x,",table[Path].DLFreg_Data[1]);
	pUSART1.printf("0x%02x,",table[Path].DLFreg_Data[2]);
	pUSART1.printf("0x%02x,",table[Path].DLFreg_Data[3]);
	pUSART1.printf("0x%02x,",table[Path].ULFreg_Sel);
	pUSART1.printf("0x%02x,",table[Path].ULFreg_Data[0]);
	pUSART1.printf("0x%02x,",table[Path].ULFreg_Data[1]);
	pUSART1.printf("0x%02x,",table[Path].ULFreg_Data[2]);
	pUSART1.printf("0x%02x,",table[Path].ULFreg_Data[3]);

	for(i = 0 ; i < table[Path].Filter_Cnt; i++)
	{
		pUSART1.printf("0x%02x,",table[Path].Filter_Data[i]);
	}
	pUSART1.printf("\n");
...
{
//	FilterOperating_2100MHz_10_10();
	FilterOperating_2100MHz_15_05();
}




void FilterOperating_2100MHz_10_10(void)
{
	INT8U i = 0;
	__Dnr_Sts *Sts = tDnrSts;
	...
	if(IsItFilter)
	{
...

		{
			...
			FilterID = Filter_2100MHz_20_10_L[nLen++];
			tIndex = FilterID - 1;
		
			table[tIndex].Flag				= SET;
			table[tIndex].Path				= FilterID;
			table[tIndex].DLFreg_Sel		= Filter_2100MHz_10_10_L[nLen++];
		
			table[tIndex].DLFreg_Data[0]	= Filter_2100MHz_10_10_L[nLen++];
			table[tIndex].DLFreg_Data[1]	= Filter_2100MHz_10_10_L[nLen++];
			table[tIndex].DLFreg_Data[2]	= Filter_2100MHz_10_10_L[nLen++];
			table[tIndex].DLFreg_Data[3]	= Filter_2100MHz_10_10_L[nLen++];
		
			table[tIndex].ULFreg_Sel		= Filter_2100MHz_10_10_L[nLen++];
		
			table[tIndex].ULFreg_Data[0]	= Filter_2100MHz_10_10_L[nLen++];
			table[tIndex].ULFreg_Data[1]	= Filter_2100MHz_10_10_L[nLen++];
			table[tIndex].ULFreg_Data[2]	= Filter_2100MHz_10_10_L[nLen++];
			table[tIndex].ULFreg_Data[3]	= Filter_2100MHz_10_10_L[nLen++];
		
			table[tIndex].Filter_Cnt		= FilterCntMax - 11;
		
			FilterCnt = table[tIndex].Filter_Cnt;
		
			for(j = 0 ; j < FilterCnt; j++)
			{
				table[tIndex].Filter_Data[j]	= Filter_2100MHz_10_10_L[nLen+j];
			}
		}
		
		{
			...
			FilterID = Filter_2100MHz_10_10_R[nLen++];
			tIndex = FilterID - 1;

			table[tIndex].Flag				= SET;
			table[tIndex].Path				= FilterID;
			table[tIndex].DLFreg_Sel		= Filter_2100MHz_10_10_R[nLen++];

			table[tIndex].DLFreg_Data[0]	= Filter_2100MHz_10_10_R[nLen++];
			table[tIndex].DLFreg_Data[1]	= Filter_2100MHz_10_10_R[nLen++];
			table[tIndex].DLFreg_Data[2]	= Filter_2100MHz_10_10_R[nLen++];
			table[tIndex].DLFreg_Data[3]	= Filter_2100MHz_10_10_R[nLen++];

			table[tIndex].ULFreg_Sel		= Filter_2100MHz_10_10_R[nLen++];

			table[tIndex].ULFreg_Data[0]	= Filter_2100MHz_10_10_R[nLen++];
			table[tIndex].ULFreg_Data[1]	= Filter_2100MHz_10_10_R[nLen++];
			table[tIndex].ULFreg_Data[2]	= Filter_2100MHz_10_10_R[nLen++];
			table[tIndex].ULFreg_Data[3]	= Filter_2100MHz_10_10_R[nLen++];
			...
			for(j = 0 ; j < FilterCnt; j++)
			{
				table[tIndex].Filter_Data[j]	= Filter_2100MHz_10_10_R[nLen+j];
			}
		}
	}
...
void FilterOperating_2100MHz_15_05(void)
{
	INT8U i = 0;
	__Dnr_Sts *Sts = tDnrSts;
	...
	if(IsItFilter)
	{
...

		{
			...
			FilterID = Filter_2100MHz_15_05_L[nLen++];
			tIndex = FilterID - 1;
		
			table[tIndex].Flag				= SET;
			table[tIndex].Path				= FilterID;
			table[tIndex].DLFreg_Sel		= Filter_2100MHz_15_05_L[nLen++];
		
			table[tIndex].DLFreg_Data[0]	= Filter_2100MHz_15_05_L[nLen++];
			table[tIndex].DLFreg_Data[1]	= Filter_2100MHz_15_05_L[nLen++];
			table[tIndex].DLFreg_Data[2]	= Filter_2100MHz_15_05_L[nLen++];
			table[tIndex].DLFreg_Data[3]	= Filter_2100MHz_15_05_L[nLen++];
		
			table[tIndex].ULFreg_Sel		= Filter_2100MHz_15_05_L[nLen++];
		
			table[tIndex].ULFreg_Data[0]	= Filter_2100MHz_15_05_L[nLen++];
			table[tIndex].ULFreg_Data[1]	= Filter_2100MHz_15_05_L[nLen++];
			table[tIndex].ULFreg_Data[2]	= Filter_2100MHz_15_05_L[nLen++];
			table[tIndex].ULFreg_Data[3]	= Filter_2100MHz_15_05_L[nLen++];
			...
			for(j = 0 ; j < FilterCnt; j++)
			{
				table[tIndex].Filter_Data[j]	= Filter_2100MHz_15_05_L[nLen+j];
			}
		}
		
		{
			...
			FilterID = Filter_2100MHz_15_05_R[nLen++];
			tIndex = FilterID - 1;

			table[tIndex].Flag				= SET;
			table[tIndex].Path				= FilterID;
			table[tIndex].DLFreg_Sel		= Filter_2100MHz_15_05_R[nLen++];

			table[tIndex].DLFreg_Data[0]	= Filter_2100MHz_15_05_R[nLen++];
			table[tIndex].DLFreg_Data[1]	= Filter_2100MHz_15_05_R[nLen++];
			table[tIndex].DLFreg_Data[2]	= Filter_2100MHz_15_05_R[nLen++];
			table[tIndex].DLFreg_Data[3]	= Filter_2100MHz_15_05_R[nLen++];

			table[tIndex].ULFreg_Sel		= Filter_2100MHz_15_05_R[nLen++];

			table[tIndex].ULFreg_Data[0]	= Filter_2100MHz_15_05_R[nLen++];
			table[tIndex].ULFreg_Data[1]	= Filter_2100MHz_15_05_R[nLen++];
			table[tIndex].ULFreg_Data[2]	= Filter_2100MHz_15_05_R[nLen++];
			table[tIndex].ULFreg_Data[3]	= Filter_2100MHz_15_05_R[nLen++];
			...
			for(j = 0 ; j < FilterCnt; j++)
			{
				table[tIndex].Filter_Data[j]	= Filter_2100MHz_15_05_R[nLen+j];
			}
		}
	}
